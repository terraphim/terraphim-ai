...
import { buildSearchQuery, parseSearchInput } from '../../src/lib/Search/searchUtils.ts';
...
describe('Search Operator Behavior', () => {
	...
	describe('Operator precedence and parsing', () => {
		...
		it('should demonstrate different search behaviors', () => {
			// This test shows that AND and OR create different query structures
			const andQuery = buildSearchQuery(parseSearchInput('rust AND javascript'), 'Engineer');
			const orQuery = buildSearchQuery(parseSearchInput('rust OR javascript'), 'Engineer');
			const singleQuery = buildSearchQuery(parseSearchInput('rust'), 'Engineer');

			// AND query should have multiple terms with 'and' operator
			expect(andQuery.operator).toBe('and');
			expect(andQuery.search_terms).toEqual(['rust', 'javascript']);

			// OR query should have multiple terms with 'or' operator
			expect(orQuery.operator).toBe('or');
			expect(orQuery.search_terms).toEqual(['rust', 'javascript']);

			// Single query should have no operator
			expect(singleQuery.operator).toBeUndefined();
			expect(singleQuery.search_terms).toBeUndefined();

			// They should create different query structures
			expect(andQuery.operator).not.toBe(orQuery.operator);
			expect(andQuery).not.toEqual(singleQuery);
			expect(orQuery).not.toEqual(singleQuery);
		});
	});
});
