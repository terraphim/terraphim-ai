...
describe('Agent Workflow Integration Tests', () => {
	...
	beforeAll(async () => {
		...
		serverProcess = spawn(
			...
			[
				...
				'--port',
				TEST_CONFIG.serverPort.toString(),
			],
			{
				stdio: 'pipe',
				cwd: process.cwd() + '/..',
	...
	describe('Workflow Execution Integration', () => {
		...
		it('should handle workflow lifecycle messages', async () => {
			...
			const workflowMessages = receivedMessages.filter(
				(msg) => msg.sessionId === sessionId || msg.session_id === sessionId
			);
		...
		it('should handle multiple concurrent workflow sessions', async () => {
			...
			const uniqueSessions = new Set(
				receivedMessages.map((msg) => msg.sessionId || msg.session_id).filter(Boolean)
			);
