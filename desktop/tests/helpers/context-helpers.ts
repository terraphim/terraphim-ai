...
export class ContextTestHelpers {
	...
	async deleteConversation(conversationId: string): Promise<void> {
		...
		await expect(
			this.page.locator(`[data-testid="conversation-${conversationId}"]`)
		).not.toBeVisible();
		await this.page.waitForLoadState('networkidle');
	}
	...
	async createConversation(data: ConversationData): Promise<string> {
		...
		const conversationId = await this.page.getAttribute(
	...
	async addSearchContext(query: string, documents: Document[], limit?: number): Promise<void> {
		...
		for (const doc of documentsToSelect) {
			...
			const docItem = this.page.locator(
				`[data-testid="search-result-item"]:has-text("${doc.title}")`
			);
		...
		const activeConversationTitle = await this.page
			.locator('[data-testid="active-conversation-title"]')
			.textContent();
	...
	async editContext(originalTitle: string, newData: Partial<ContextData>): Promise<void> {
		const contextItem = this.page.locator(
			`[data-testid="context-item"]:has-text("${originalTitle}")`
		);
	...
	async removeContext(contextTitle: string): Promise<void> {
		const contextItem = this.page.locator(
			`[data-testid="context-item"]:has-text("${contextTitle}")`
		);
	...
	async assertConversationExists(title: string): Promise<void> {
		await this.navigateToConversations();
		await expect(
			this.page.locator(`[data-testid="conversation-item"]:has-text("${title}")`)
		).toBeVisible();
	}
	...
	async assertConversationTitle(expectedTitle: string): Promise<void> {
		await expect(this.page.locator('[data-testid="active-conversation-title"]')).toHaveText(
			expectedTitle
		);
	}

	async assertConversationRole(expectedRole: string): Promise<void> {
		await expect(this.page.locator('[data-testid="active-conversation-role"]')).toContainText(
			expectedRole
		);
	}
