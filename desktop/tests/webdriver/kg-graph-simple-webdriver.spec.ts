...
test.describe('KG Graph Functionality - Simple WebDriver Test', () => {
  ...
  test('should prove KG graph functionality with simple WebDriver approach', async ({ page }) => {
    ...
    if (svgVisible) {
      ...
      if (nodeCount > 0) {
        ...
        if (modalVisible) {
          ...
          const kgContext = page.locator('.kg-context, .tag.is-info, .tag:has-text("kg"), .tag:has-text("knowledge")');
    ...
    const kgTags = page.locator('.tag-button, .tag[role="button"], button .tag, .tag:has-text("kg"), .tag:has-text("knowledge")');
    ...
    if (isTauri) {
      ...
      try {
        const tauriCommands = await page.evaluate(() => {
          if (typeof (window as any).__TAURI__ !== 'undefined' && (window as any).__TAURI__.invoke) {
            return true;
          }
          return false;
        });
