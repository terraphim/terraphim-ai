...
test.describe('Logical Operators Search E2E', () => {
	...
	test.describe('UI Search with Logical Operators', () => {
		test('should parse AND operator in search input', async ({ page }) => {
			...
			const searchInput = page
				.locator('input[placeholder*="search"], input[type="search"], .search-input')
				.first();
			...
			await Promise.race([
				...
				page
					.locator('button[type="submit"], .search-button, button:has-text("Search")')
					.first()
					.click()
					.catch(() => {}),
			]);
	...
	test.describe('Autocomplete with Logical Operators', () => {
		test('should suggest AND and OR operators after typing a term', async ({ page }) => {
			...
			const andSuggestion = page.locator(
				'[data-testid="autocomplete-suggestion"]:has-text("rust AND")'
			);
			const orSuggestion = page.locator(
				'[data-testid="autocomplete-suggestion"]:has-text("rust OR")'
			);

			// At least one operator suggestion should be present
			const hasOperatorSuggestions =
				(await andSuggestion.isVisible()) || (await orSuggestion.isVisible());
		...
		test('should suggest second terms after typing operator', async ({ page }) => {
			...
			const hasTermSuggestions = suggestionTexts.some(
				(text) => !text.includes('AND') && !text.includes('OR')
			);
	...
	test.describe('Error Handling', () => {
		...
		test('should handle empty results gracefully', async ({ page }) => {
			...
			const hasNoResultsIndication =
				(await noResultsMessage.isVisible()) || (await emptyState.isVisible());
		...
		test('should handle service errors gracefully', async ({ page }) => {
			...
			const errorMessage = page.locator(
				'[data-testid="error-message"], [data-testid="service-error"]'
			);
	...
	test.describe('Performance and UX', () => {
		test('should show loading state during search', async ({ page }) => {
			...
			const loadingIndicator = page.locator(
				'[data-testid="loading"], [data-testid="search-loading"]'
			);
