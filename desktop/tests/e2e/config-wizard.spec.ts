...
test.describe('Configuration Wizard', () => {
	...
	test('should allow editing global configuration settings', async ({ page }) => {
		...
		await themeInput.selectOption('superhero');
	...
	test('should allow adding and configuring roles', async ({ page }) => {
		...
		await roleThemeInput.selectOption('lumen');
		await expect(roleThemeInput).toHaveValue('lumen');
		const relevanceInput = page.locator('#role-relevance-0');
		await relevanceInput.selectOption('terraphim-graph');
		await expect(relevanceInput).toHaveValue('terraphim-graph');
	});
	...
	test('should save configuration and update via API', async ({ page }) => {
		...
		const themeInput = page.locator('#default-theme');
		await themeInput.selectOption('spacelab');
		...
		const relevanceInput = page.locator('#role-relevance-0');
		await relevanceInput.selectOption('title-scorer');
	...
	test('should handle complex role configurations', async ({ page }) => {
		...
		await secondRoleThemeInput.selectOption('superhero');

		const secondRelevanceInput = page.locator('#role-relevance-1');
		await secondRelevanceInput.selectOption('title-scorer');
	...
	// === Relevance Function Select Field Tests ===
	test('should display relevance function as select dropdown with all available options', async ({
		page,
	}) => {
		await page.goto('/config/wizard');
		await page.getByTestId('wizard-next').click(); // Go to roles step
		await page.getByTestId('add-role').click(); // Add a role
		await page.waitForSelector('#role-name-0', { timeout: 5000 });

		// Check that relevance function is a select field
		const relevanceSelect = page.locator('#role-relevance-0');
		await expect(relevanceSelect).toBeVisible();

		// Verify it's a select element
		const tagName = await relevanceSelect.evaluate((el) => el.tagName.toLowerCase());
		expect(tagName).toBe('select');

		// Check all available options
		const options = await relevanceSelect.locator('option').allTextContents();
		expect(options).toEqual(
			expect.arrayContaining(['terraphim-graph', 'title-scorer', 'bm25', 'bm25f', 'bm25plus'])
		);
	});

	test('should allow selecting different relevance function options', async ({ page }) => {
		await page.goto('/config/wizard');
		await page.getByTestId('wizard-next').click(); // Go to roles step
		await page.getByTestId('add-role').click(); // Add a role
		await page.waitForSelector('#role-name-0', { timeout: 5000 });
		...
		const expectedOptions = ['terraphim-graph', 'title-scorer', 'bm25', 'bm25f', 'bm25plus'];

		for (const option of expectedOptions) {
			await relevanceSelect.selectOption(option);
			await expect(relevanceSelect).toHaveValue(option);
		}
	});

	test('should have default relevance function value selected', async ({ page }) => {
		await page.goto('/config/wizard');
		await page.getByTestId('wizard-next').click(); // Go to roles step
		await page.getByTestId('add-role').click(); // Add a role
		await page.waitForSelector('#role-name-0', { timeout: 5000 });
		...
		// Should have the default value from the form initialization
		const defaultValue = await relevanceSelect.inputValue();
		expect(defaultValue).toBe('title-scorer'); // Based on line 269 in ConfigWizard.svelte
	});

	test('should preserve selected relevance function when navigating between steps', async ({
		page,
	}) => {
		await page.goto('/config/wizard');
		await page.getByTestId('wizard-next').click(); // Go to roles step
		...
		await relevanceSelect.selectOption('bm25plus');
		await expect(relevanceSelect).toHaveValue('bm25plus');

		// Navigate to review step and back
		await page.getByTestId('wizard-next').click(); // review
		await page.getByTestId('wizard-back').click(); // back to roles

		// Value should be preserved
		const relevanceSelectAfterNavigation = page.locator('#role-relevance-0');
		await expect(relevanceSelectAfterNavigation).toHaveValue('bm25plus');
	});
});
