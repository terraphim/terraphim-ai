...
test.describe('Configuration Wizard', () => {
  ...
  test('should display relevance function as select dropdown with all available options', async ({ page }) => {
    await page.goto('/config/wizard');
    await page.getByTestId('wizard-next').click(); // Go to roles step
  ...
  test('should preserve selected relevance function when navigating between steps', async ({ page }) => {
    await page.goto('/config/wizard');
    await page.getByTestId('wizard-next').click(); // Go to roles step
    await page.getByTestId('add-role').click(); // Add a role
    await page.waitForSelector('#role-name-0', { timeout: 5000 });
