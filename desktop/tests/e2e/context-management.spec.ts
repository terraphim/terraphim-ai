...
test.describe('Context Management UI', () => {
  ...
  test.describe('Conversation Management', () => {
    test('should create new conversation', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="active-conversation-title"]')).toContainText('Test Conversation');
      await expect(page.locator('[data-testid="active-conversation-role"]')).toContainText('Engineer');
    });
  ...
  test.describe('Message Management', () => {
    ...
    test('should add user message to conversation', async ({ page }) => {
      ...
      const userMessage = page.locator('[data-testid="message-item"]:has-text("' + testMessage + '")');
    ...
    test('should support different message roles', async ({ page }) => {
      ...
      for (const message of messages) {
        ...
        const messageItem = page.locator(`[data-testid="message-item"]:has-text("${message.content}")`);
  ...
  test.describe('Context Management', () => {
    ...
    test('should add search results as context', async ({ page }) => {
      ...
      await page.selectOption('[data-testid="target-conversation-select"]', 'Context Test Conversation');
    ...
    test('should add manual context', async ({ page }) => {
      ...
      await page.fill('[data-testid="context-content-textarea"]', 'I am a beginner Rust programmer learning async concepts.');
    ...
    test('should display different context types with appropriate styling', async ({ page }) => {
      ...
      for (const context of contextTypes) {
        ...
        const contextItem = page.locator(`[data-testid="context-item"]:has-text("${context.title}")`);
        await expect(contextItem).toHaveAttribute('data-context-type', context.type);
        await expect(contextItem.locator(`[data-testid="context-icon"]`)).toHaveClass(new RegExp(context.icon));
      }
    });
    ...
    test('should edit existing context', async ({ page }) => {
      ...
      // Verify changes
      await expect(page.locator('[data-testid="context-item"]:has-text("Updated Title")')).toBeVisible();
    ...
    test('should remove context items', async ({ page }) => {
      ...
      const contextItem = page.locator('[data-testid="context-item"]:has-text("Context to Remove")');
      await expect(contextItem).toBeVisible();
  ...
  test.describe('Context Integration with Chat', () => {
    test('should show context information in chat interface', async ({ page }) => {
      ...
      await page.fill('[data-testid="context-content-textarea"]', 'Important information for the AI assistant');
      await page.click('[data-testid="save-context-button"]');

      // Start chat or send message
      await page.fill('[data-testid="message-input"]', 'Please help me based on the provided context');
      await page.click('[data-testid="send-message-button"]');
    ...
    test('should highlight relevant context during conversation', async ({ page }) => {
      ...
      const contexts = [
        ...
        { title: 'Async Programming', content: 'Asynchronous programming patterns', relevant: true }
      ];
      ...
      const asyncContext = page.locator('[data-testid="context-item"]:has-text("Async Programming")');
  ...
  test.describe('Context Limits and Validation', () => {
    ...
    test('should validate context input', async ({ page }) => {
      ...
      // Should succeed now
      await expect(page.locator('[data-testid="context-item"]:has-text("Valid Title")')).toBeVisible();
    });
  });
  ...
  test.describe('Accessibility and Keyboard Navigation', () => {
    test('should support keyboard navigation for context management', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="context-item"]:has-text("Keyboard Context")')).toBeVisible();
    });
    ...
    test('should have proper ARIA labels and roles', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="conversation-messages"]')).toHaveAttribute('role', 'log');
      await expect(page.locator('[data-testid="conversation-context"]')).toHaveAttribute('role', 'region');
      await expect(page.locator('[data-testid="conversation-context"]')).toHaveAttribute('aria-label', /context/i);

      // Check buttons have proper labels
      await expect(page.locator('[data-testid="add-manual-context-button"]')).toHaveAttribute('aria-label');
      await expect(page.locator('[data-testid="send-message-button"]')).toHaveAttribute('aria-label');
    });
  });
});
...
test.describe('Context Management Integration Tests', () => {
  test('should integrate context with backend Tauri commands', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="context-item"]:has-text("Backend Test Context")')).toBeVisible();
  });
});
