...
const MOCK_THESAURUS_CONTENT = {
  ...
  "tokio": {
    ...
    "definition": "Asynchronous runtime for Rust providing I/O, networking, and concurrency primitives",
...
test.describe('KG Thesaurus Content Addition E2E', () => {
  ...
  test.describe('Thesaurus Content Addition', () => {
    test('should add complete thesaurus JSON content to context', async ({ page }) => {
      ...
      const addThesaurusButton = page.locator('button:has-text("Add Complete Thesaurus to Context")');
      ...
      await expect(contextPanel).toContainText('Asynchronous programming paradigm', { timeout: 3000 });
    ...
    test('should verify thesaurus content is searchable and usable by AI', async ({ page }) => {
      ...
      await expect(contextItem).toContainText('comprehensive vocabulary context', { timeout: 3000 });
