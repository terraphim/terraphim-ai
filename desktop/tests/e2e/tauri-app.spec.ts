...
test.describe('Tauri App - Complete Functionality', () => {
  ...
  test.describe('Configuration Wizard Screen', () => {
    ...
    test('should configure knowledge graph settings', async ({ page }) => {
      ...
      await expect(page.locator('#kg-url-0')).toHaveValue('https://staging-storage.terraphim.io/thesaurus_Default.json');
    ...
    test('should save configuration successfully', async ({ page }) => {
      ...
      const successMessage = page.locator('.success, .alert-success, [data-testid="wizard-success"]');
  ...
  test.describe('Graph Visualization Screen', () => {
    ...
    test('should handle graph node dragging', async ({ page }) => {
      ...
      if (initialBox && newBox) {
        const moved = Math.abs(initialBox.x - newBox.x) > 5 || Math.abs(initialBox.y - newBox.y) > 5;
  ...
  test.describe('Performance and Stability', () => {
    ...
    test('should handle large search queries', async ({ page }) => {
      ...
      const longQuery = 'artificial intelligence machine learning deep learning neural networks computer vision natural language processing data science algorithms optimization'.repeat(5);
