...
test.describe('Atomic Server Haystack Integration', () => {
	...
	test('should perform atomic haystack search and return results', async () => {
		...
		console.log(
			`📊 Search summary: ${successfulSearches}/${searchTerms.length} successful searches, ${totalResults} total results`
		);
	...
	test('should validate dual haystack configuration and search', async () => {
		...
		if (searchResponse.ok) {
			...
			if (results.results && results.results.length > 0) {
				...
				const atomicResults = results.results.filter(
					(doc) => doc.url && doc.url.includes('localhost:9883')
				);
				const ripgrepResults = results.results.filter(
					(doc) => doc.url && !doc.url.includes('localhost:9883')
				);

				console.log(
					`📄 Atomic results: ${atomicResults.length}, Ripgrep results: ${ripgrepResults.length}`
				);
			}
		}
