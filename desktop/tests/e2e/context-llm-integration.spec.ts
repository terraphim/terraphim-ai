...
const TEST_DOCUMENTS = [
  {
    ...
    content: 'Rust async programming uses the async/await syntax and the tokio runtime. Key concepts include futures, streams, and error handling with Result types.',
    tags: ['rust', 'async', 'tokio', 'futures']
  },
  {
    title: 'Error Handling in Rust',
    content: 'Rust uses Result<T, E> and Option<T> types for error handling. The ? operator is used for early returns, and match expressions handle different error cases.',
    tags: ['rust', 'error-handling', 'result', 'option']
  },
  {
    title: 'Tokio Runtime Concepts',
    content: 'The Tokio runtime provides async I/O, timers, and task scheduling. It uses a work-stealing scheduler and supports both single-threaded and multi-threaded execution.',
...
test.describe('Context and LLM Integration Tests', () => {
  ...
  test.describe('Knowledge Graph Context Integration', () => {
    ...
    test('should handle mixed document and KG context', async ({ page }) => {
      ...
      await testLLMResponseWithContext(page, 'How do the documents and KG terms relate to each other?');
