  ...
  test.describe('Basic Summarization Functionality', () => {
    test('should generate AI summary for search result', async ({ page }) => {
      ...
      if (resultCount > 0) {
        ...
        const summarizeButton = firstResult.locator('[data-testid="summarize-button"], .summarize-action, button:has-text("Summarize")');
        ...
        if (summarizeButtonVisible) {
          ...
          const loadingIndicator = firstResult.locator('[data-testid="summary-loading"], .summary-loading');
          if (await loadingIndicator.isVisible()) {
            console.log('Summary generation started...');

            // Wait for summary to appear or loading to disappear
            await page.waitForFunction(() => {
              const loading = document.querySelector('[data-testid="summary-loading"], .summary-loading');
          ...
          const summaryContent = firstResult.locator('[data-testid="ai-summary"], .ai-summary-content');
          ...
          if (summaryVisible) {
            ...
            const errorMessage = firstResult.locator('[data-testid="summary-error"], .summary-error');
    ...
    test('should handle multiple document summarization', async ({ page }) => {
      ...
      if (resultCount >= 2) {
        ...
        for (let i = 0; i < Math.min(2, resultCount); i++) {
          ...
          const summarizeButton = resultItem.locator('[data-testid="summarize-button"], button:has-text("Summarize")');
    ...
    test('should show cache status for summaries', async ({ page }) => {
      ...
      const summarizeButton = firstResult.locator('[data-testid="summarize-button"], button:has-text("Summarize")');
      const buttonVisible = await summarizeButton.isVisible();
      ...
      if (buttonVisible) {
        ...
        const cacheIndicator = firstResult.locator('[data-testid="cache-status"], .cache-indicator, .tag:has-text("cached"), .tag:has-text("fresh")');
        ...
        const regenerateButton = firstResult.locator('[data-testid="regenerate-summary"], button:has-text("Regenerate")');
  ...
  test.describe('Auto-Summarization Configuration', () => {
    test('should respect auto-summarize setting in role configuration', async ({ page }) => {
      ...
      if (roleCount > 0) {
        ...
        const autoSummarizeCheckbox = firstRole.locator('input[type="checkbox"]:near(text("auto-summarize")), input[id*="auto-summarize"]');
        ...
        if (hasAutoSummarize) {
          ...
          if (canSave) {
            ...
            const resultWithSummary = page.locator('.result-item:has([data-testid="ai-summary"], .ai-summary)');
    ...
    test('should validate LLM provider configuration for summarization', async ({ page }) => {
      ...
      const llmProviderSelect = page.locator('select[name*="llm"], select:has(option:text("ollama"))');
      ...
      if (hasLlmConfig) {
        ...
        if (hasOllama) {
          ...
          const ollamaUrlInput = page.locator('input[name*="ollama_base_url"], input[placeholder*="ollama"]');
          const ollamaModelInput = page.locator('input[name*="ollama_model"], input[value*="llama"]');
  ...
  test.describe('Error Handling and Edge Cases', () => {
      ...
      if (buttonVisible) {
        ...
        try {
          ...
          if (hasError) {
            ...
            const retryButton = firstResult.locator('[data-testid="retry-summary"], button:has-text("Retry")');
    ...
    test('should handle Ollama service unavailable', async ({ page }) => {
      ...
      if (buttonVisible) {
        ...
        if (hasError) {
          const errorText = await firstResult.locator('[data-testid="summary-error"]').textContent();
    ...
    test('should handle empty or short documents', async ({ page }) => {
      ...
      if (resultCount > 0) {
        ...
        const contentElement = result.locator('.description, .content, [data-testid="document-body"]');
  ...
  test.describe('Performance and User Experience', () => {
    test('should provide visual feedback during summarization', async ({ page }) => {
      ...
      if (buttonVisible) {
        ...
        const loadingIndicator = firstResult.locator('[data-testid="summary-loading"], .loading, .spinner');
    ...
    test('should allow canceling summarization', async ({ page }) => {
      ...
      if (buttonVisible) {
        ...
        const cancelButton = firstResult.locator('[data-testid="cancel-summary"], button:has-text("Cancel")');
        ...
        if (canCancel) {
          ...
          const stillLoading = await firstResult.locator('[data-testid="summary-loading"]').isVisible();
          expect(stillLoading).toBeFalsy();
