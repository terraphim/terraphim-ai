...
test.describe('Context Management Performance Tests @performance', () => {
		...
		await test.step('Measure context addition performance', async () => {
			...
			for (let i = 0; i < LARGE_DATASET_SIZE; i++) {
				...
				await helpers.addManualContext({
					...
					content:
						`Large content block for performance testing. This is item number ${i + 1} of ${LARGE_DATASET_SIZE}. `.repeat(
							50
						),
			...
			// Analyze performance
			const averageAddTime =
				additionMetrics.reduce((sum, m) => sum + m.duration, 0) / additionMetrics.length;
	...
	test('Message handling performance with conversation history', async ({ page }) => {
		...
		await test.step('Measure message addition performance', async () => {
			...
			const averageTime =
				messageMetrics.reduce((sum, m) => sum + m.duration, 0) / messageMetrics.length;
		...
		await test.step('Perform rapid conversation switching', async () => {
			...
			for (let round = 0; round < 50; round++) {
				...
				await expect(page.locator('[data-testid="current-conversation-title"]')).toHaveText(
					`Switch Test Conversation ${randomIndex + 1}`
				);
		...
		await test.step('Add extremely large context items', async () => {
			...
			for (const [index, size] of largeContentSizes.entries()) {
				...
				try {
					...
					console.log(
						`Large content ${index + 1} failed (expected for very large content):`,
						error
					);
