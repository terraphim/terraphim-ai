	...
	await page.waitForFunction(
		() => {
			...
			const items = dropdown?.querySelectorAll(
				'.terraphim-suggestion-item:not(.terraphim-suggestion-empty)'
			);
...
export async function getAutocompleteSuggestions(page: Page): Promise<AutocompleteSuggestion[]> {
	...
	for (let i = 0; i < count; i++) {
		...
		const snippet =
			(await item.locator('.terraphim-suggestion-snippet').textContent()) || undefined;
...
export async function clickAutocompleteRebuildButton(page: Page): Promise<void> {
	const rebuildButton = await ciWaitForSelector(
		page,
		'[data-testid="autocomplete-rebuild-button"]'
	);
...
export async function waitForAutocompleteStatus(
	...
	await page.waitForFunction(
		(status) => {
			...
			return (
				statusElement && statusElement.textContent && statusElement.textContent.includes(status)
			);
		},
		expectedStatus,
		{ timeout }
 ...
 */
export async function screenshotAutocompleteDropdown(page: Page, filename: string): Promise<void> {
	const dropdown = page.locator('.terraphim-suggestion-dropdown');
	await expect(dropdown).toBeVisible();
