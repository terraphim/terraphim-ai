		...
		try {
			...
			for (const { term, description } of searchTerms) {
				...
				if (searchResponse.ok) {
					...
					if (contentType && contentType.includes('application/json')) {
						try {
							...
							if (searchResults && searchResults.results && Array.isArray(searchResults.results)) {
								...
								console.log(
									`⚠️ Unexpected result structure for "${term}":`,
					...
					console.log(
						`❌ Search failed for "${term}" (${searchResponse.status}):`,
			...
			console.log(
				`📊 SEARCH SUMMARY: ${successfulSearches}/${searchTerms.length} searches successful`
			);
			...
			console.log(
				'✅ ATOMIC HAYSTACK INTEGRATION SUCCESSFUL - Documents returned from atomic server!'
			);
			...
			if (totalResults > 0) {
				...
				console.log(
					'ℹ️ Integration working but no document content returned (may be authentication or indexing issue)'
				);
			}
		} catch (error) {
