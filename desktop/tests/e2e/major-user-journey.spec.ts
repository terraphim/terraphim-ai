...
// Test data
const TEST_SEARCH_QUERIES = ['rust async programming', 'tokio futures', 'error handling patterns'];
...
const TEST_KG_TERMS = ['async-await', 'futures', 'tokio-runtime'];
...
test.describe('Major User Journey E2E Tests', () => {
	...
	test.describe('Complete User Journey', () => {
		test('should complete full user journey: search → add context → chat → add KG → chat with enhanced context', async ({
			page,
		}) => {
			...
			const contextItems = contextPanel.locator(
				'[data-testid="conversation-context"] .context-item'
			);
		...
		test('should handle multiple documents in context', async ({ page }) => {
			...
			await performChatWithContext(
				page,
				'Summarize the key points from all the documents in context'
			);
	...
	async function verifyContextPassedToLLM(page: any) {
		...
		await chatInput.fill(
			'Please reference the specific documents and terms I added to context in your response'
		);
		await sendButton.click();
