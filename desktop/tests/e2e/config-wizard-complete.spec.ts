...
test.describe('Configuration Wizard Complete E2E Tests', () => {
	...
	test.describe('LLM Provider Configuration', () => {
		test('should configure Ollama provider with validation', async ({ page }) => {
			...
			if (roleCount > 0) {
				...
				const llmProviderSelect = firstRole.locator(
				...
				if (llmControl) {
					...
					const baseUrlInput = firstRole.locator(
						'input[name*="ollama_base_url"], input[name*="base_url"], input[placeholder*="http://127.0.0.1:11434"]'
					);
					...
					const modelInput = firstRole.locator(
						'input[name*="ollama_model"], input[name*="model"], input[placeholder*="llama"]'
					);
					...
					const autoSummarizeCheckbox = firstRole.locator(
						'input[type="checkbox"][name*="auto_summarize"], input[type="checkbox"]:near(text("auto-summarize"))'
					);
					...
					const testConnectionButton = firstRole.locator(
						'[data-testid="test-ollama-connection"], button:has-text("Test Connection"), button:has-text("Validate")'
					);
					if (await testConnectionButton.isVisible()) {
						...
						await page.waitForFunction(
							() => {
								const successEl = document.querySelector(
									'.connection-success, .text-success, [data-testid="connection-success"]'
								);
								const errorEl = document.querySelector(
									'.connection-error, .text-danger, [data-testid="connection-error"]'
								);
						...
						const connectionSuccess = await firstRole
							.locator('.connection-success, .text-success, [data-testid="connection-success"]')
							.isVisible();
						const connectionError = await firstRole
							.locator('.connection-error, .text-danger, [data-testid="connection-error"]')
							.isVisible();

						if (connectionSuccess) {
							console.log('Ollama connection test successful');
						} else if (connectionError) {
							const errorText = await firstRole
								.locator('.connection-error, .text-danger')
								.textContent();
					...
					const saveButton = page.locator(
						'[data-testid="save-config"], button:has-text("Save"), button:has-text("Apply")'
					);
					if (await saveButton.isVisible()) {
						...
						await page.waitForFunction(
							() => {
								const successMsg = document.querySelector(
									'.save-success, .alert-success, [data-testid="save-success"]'
								);
								const errorMsg = document.querySelector(
									'.save-error, .alert-danger, [data-testid="save-error"]'
								);
						...
						const saveSuccess = await page
							.locator('.save-success, .alert-success, [data-testid="save-success"]')
							.isVisible();
		...
		test('should configure OpenRouter provider with API key validation', async ({ page }) => {
			...
			if (roleCount > 1) {
				...
				const apiKeyInput = secondRole.locator(
					'input[name*="openrouter_api_key"], input[name*="api_key"], input[type="password"]'
				);
				...
				// Configure model
				const modelInput = secondRole.locator(
					'input[name*="openrouter_model"], input[name*="model"]:not([name*="ollama"])'
				);
				...
				// Configure auto-summarize
				const autoSummarizeCheckbox = secondRole.locator(
					'input[type="checkbox"][name*="openrouter_auto_summarize"], input[type="checkbox"]:near(text("auto-summarize"))'
				);
				if (await autoSummarizeCheckbox.isVisible()) {
					const isChecked = await autoSummarizeCheckbox.isChecked();
				...
				const validateButton = secondRole.locator(
					'[data-testid="validate-openrouter"], button:has-text("Validate"), button:has-text("Test")'
				);
				if (await validateButton.isVisible()) {
					...
					const validationSuccess = await secondRole
						.locator('.validation-success, .text-success')
						.isVisible();
					const validationError = await secondRole
						.locator('.validation-error, .text-danger')
						.isVisible();

					if (validationSuccess) {
						console.log('OpenRouter API key validation successful');
					} else if (validationError) {
						const errorText = await secondRole
							.locator('.validation-error, .text-danger')
							.textContent();
	...
	test.describe('Haystack Configuration with Secrets', () => {
		test('should configure Atomic Server haystack with secrets', async ({ page }) => {
			...
			const haystackSection = page.locator(
				'[data-testid="haystack-config"], .haystack-config, [data-testid="haystack-section"]'
			);

			if (await haystackSection.isVisible()) {
				// Add new haystack if button is available
				const addHaystackButton = page.locator(
					'[data-testid="add-haystack"], button:has-text("Add Haystack")'
				);
				...
				if (await haystackForm.isVisible()) {
					...
					const serviceSelect = haystackForm.locator(
						'select[name*="service"], select:has(option:text("AtomicServer"))'
					);
					...
					const urlInput = haystackForm.locator(
						'input[name*="atomic_server_url"], input[name*="url"], input[placeholder*="atomic"]'
					);
					...
					// Configure secret
					const secretInput = haystackForm.locator(
						'input[name*="atomic_server_secret"], input[name*="secret"], input[type="password"]'
					);
					...
					// Test connection
					const testConnectionButton = haystackForm.locator(
						'[data-testid="test-atomic-connection"], button:has-text("Test Connection")'
					);
					if (await testConnectionButton.isVisible()) {
		...
		test('should configure ClickUp haystack with API token', async ({ page }) => {
			...
			if (await haystackSection.isVisible()) {
				const addHaystackButton = page.locator(
					'[data-testid="add-haystack"], button:has-text("Add")'
				);
				if (await addHaystackButton.isVisible()) {
					await addHaystackButton.click();
					await ciWait(page, 'small');
				...
				// Configure API token
				const tokenInput = haystackForm.locator(
					'input[name*="clickup_api_token"], input[name*="api_token"], input[placeholder*="pk_"]'
				);
				...
				// Configure team ID
				const teamIdInput = haystackForm.locator(
					'input[name*="clickup_team_id"], input[name*="team_id"]'
				);
				...
				// Test API token
				const validateTokenButton = haystackForm.locator(
					'[data-testid="validate-clickup-token"], button:has-text("Validate Token")'
				);
		...
		test('should handle environment variable integration', async ({ page }) => {
			...
			const secretInputs = page.locator(
				'input[type="password"], input[name*="secret"], input[name*="token"]'
			);
	...
	test.describe('Role Management', () => {
		test('should create new role with complete configuration', async ({ page }) => {
			...
			const addRoleButton = page.locator(
				'[data-testid="add-role"], button:has-text("Add Role"), button:has-text("New Role")'
			);
			...
			if (await addRoleButton.isVisible()) {
				...
				const relevanceFunctionSelect = roleForm.locator(
					'select[name*="relevance_function"], select[name*="relevance"]'
				);
				...
				if (await llmProviderSelect.isVisible()) {
					...
					// Configure auto-summarize
					const autoSummarizeCheckbox = roleForm.locator(
				...
				// Save role
				const saveRoleButton = roleForm.locator(
					'[data-testid="save-role"], button:has-text("Save Role")'
				);
				const saveButton = page.locator('[data-testid="save-config"], button:has-text("Save")');
		...
		test('should delete role configuration', async ({ page }) => {
			...
			if (roleCount > 1) {
				...
				const deleteButton = lastRole.locator(
					'[data-testid="delete-role"], button:has-text("Delete"), .delete-role-button'
				);

				if (await deleteButton.isVisible()) {
					// Get role name before deletion
					const roleNameElement = lastRole.locator(
						'input[name*="name"], [data-testid="role-name"]'
					);
					...
					const confirmButton = page.locator(
						'[data-testid="confirm-delete-button"], button:has-text("Confirm"), button:has-text("Delete")'
					);
		...
		test('should export and import configuration', async ({ page }) => {
			...
			const exportButton = page.locator(
				'[data-testid="export-config"], button:has-text("Export"), .export-button'
			);
			...
			if (await exportButton.isVisible()) {
				...
				if (await downloadModal.isVisible()) {
					...
					const downloadButton = downloadModal.locator(
						'[data-testid="download-config"], button:has-text("Download")'
					);
				...
				const importButton = page.locator(
					'[data-testid="import-config"], button:has-text("Import"), .import-button'
				);
				...
				if (await importButton.isVisible()) {
					...
					if (await importModal.isVisible()) {
						...
						const cancelButton = importModal.locator(
							'[data-testid="cancel-import"], button:has-text("Cancel")'
						);
	...
	test.describe('Configuration Validation and Persistence', () => {
		test('should persist configuration changes across page reloads', async ({ page }) => {
			...
			if (await themeSelect.isVisible()) {
				...
				const saveButtonRestore = page.locator(
			...
			if (await addRoleButton.isVisible()) {
				...
				if (await newNameInput.isVisible()) {
					...
					const conflictError = page.locator(
						'.conflict-error, .duplicate-error, .validation-error'
					);
