...
test.describe('KG Graph Functionality - Simple WebDriver Test', () => {
	...
	test('should prove KG graph functionality with simple WebDriver approach', async ({ page }) => {
		...
		let graphLink = page.locator(
			'a[href="/graph"], a[href*="graph"], button:has-text("Graph"), .nav-link:has-text("Graph")'
		);
		...
		// If no footer navigation, try direct URL navigation
		if (!(await graphLink.count())) {
			console.log('🔗 Navigating directly to graph page...');
			await page.goto('/graph');
		} else {
			console.log('🔗 Clicking graph navigation link...');
		...
		if (svgVisible) {
			...
			if (nodeCount > 0) {
				...
				const modal = page.locator('.modal.is-active, .modal-content, .article-modal');
				...
				if (modalVisible) {
					...
					const kgContext = page.locator(
						'.kg-context, .tag.is-info, .tag:has-text("kg"), .tag:has-text("knowledge")'
					);
					...
					// Close modal
					const closeButton = page.locator('.modal-close, .delete, .close-button').first();
				...
				// Check for edit modal or context menu
				const editModal = page.locator('.modal.is-active, .modal-content, .context-menu');
				const editModalVisible = await editModal.isVisible({ timeout: 5000 }).catch(() => false);

				if (editModalVisible) {
					console.log('✅ Node right-click opened edit modal/context menu');
		...
		const kgTags = page.locator(
			'.tag-button, .tag[role="button"], button .tag, .tag:has-text("kg"), .tag:has-text("knowledge")'
		);
		...
		if (tagCount > 0) {
			...
			const kgModal = page.locator('.modal.is-active, .modal-content, .article-modal');
		...
		// Test Tauri-specific functionality
		console.log('🔧 Testing Tauri-specific functionality...');
		...
		if (isTauri) {
			...
			try {
				...
				console.log('ℹ️ Tauri commands not accessible from test context');
			}
		} else {
			console.log('ℹ️ Not running in Tauri environment (web mode)');
		}

		console.log('🎉 KG Graph Functionality Simple WebDriver Test Complete!');
		console.log('');
		console.log('📋 SUMMARY:');
