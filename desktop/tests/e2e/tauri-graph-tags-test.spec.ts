...
test.describe('Tauri App - Graph Tags Functionality', () => {
  ...
  test('should handle KG link clicks in document content', async ({ page }) => {
    ...
    await expect(page.locator('.modal.is-active, .modal-content').first()).toBeVisible({ timeout: 5000 });
    ...
    if (await kgLinks.count() > 0) {
      ...
      const hasKgModal = await page.locator('.modal.is-active .modal.is-active, .modal-content .modal-content').first().isVisible({ timeout: 5000 }).catch(() => false);
