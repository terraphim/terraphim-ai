...
test.describe('Terraphim Engineer - Complete Functionality Test', () => {
	test.beforeAll(async () => {
		...
		if (!atomicSecret) {
			console.warn(
				'⚠️  Warning: ATOMIC_SERVER_SECRET not set - atomic save functionality will be limited'
			);
		}
	...
	test('should start Terraphim server with test configuration', async ({ page }) => {
		...
		await expect(
			page.locator(
				'[data-testid="search-container"], .search-container, input[type="search"], #search-input'
			)
	...
	test('should test AI summarization functionality', async ({ page }) => {
		...
		const aiSummaryButton = page
			.locator('button:has-text("AI Summary"), .ai-summary-button, button[title*="summary"]')
			.first();
		...
		if ((await aiSummaryButton.count()) > 0) {
			...
			await expect(
				page.locator('.ai-summary-loading, [class*="loading"], .fa-spinner').first()
			).toBeVisible({ timeout: 5000 });

			// Wait for summary content
			await expect(
				page.locator('.ai-summary-content, .ai-summary, [class*="summary-content"]').first()
			...
			const summaryContent = await page
				.locator('.ai-summary-content, .ai-summary')
				.first()
				.textContent();
	...
	test('should test atomic save functionality', async ({ page }) => {
		...
		const atomicSaveButton = page
			.locator('button[aria-label*="Atomic"], .fa-cloud-upload-alt, button[title*="Atomic"]')
			.first();
		...
		if ((await atomicSaveButton.count()) > 0) {
			...
			await expect(
				page.locator('.modal.is-active, .modal-content, [class*="modal"]').first()
			).toBeVisible({ timeout: 10000 });
			...
			const articleTitleInput = page
				.locator('input[placeholder*="title"], #article-title, [name="title"]')
				.first();
			...
			if ((await articleTitleInput.count()) > 0) {
				...
				const closeButton = page
					.locator('.modal-close, .delete, button:has-text("Close"), button:has-text("Cancel")')
					.first();
			...
			console.log(
				'⚠️  Atomic save button not found - may not be available for current role configuration'
			);
		}
	});
	...
	test('should validate complete workflow end-to-end', async ({ page }) => {
		...
		if (testConfigInfo.hasAtomicSecret) {
			...
			const atomicButton = page
				.locator('button[aria-label*="Atomic"], .fa-cloud-upload-alt')
				.first();
		...
		console.log(
			`  🤖 AI Summary: ${testConfigInfo.hasOpenRouterKey ? '✅ Available' : '⚠️  Not configured'}`
		);
		console.log(
			`  ☁️  Atomic Save: ${testConfigInfo.hasAtomicSecret ? '✅ Available' : '⚠️  Not configured'}`
		);
...
test.describe('Configuration System Validation', () => {
	test('should validate environment variable substitution', async () => {
		...
		const templateExists = fs.existsSync(
			path.join(PROJECT_ROOT, 'terraphim_engineer_test_config.json')
		);
