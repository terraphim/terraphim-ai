			...
			contexts: [
				{
					...
					content:
						'Comprehensive guide to async programming patterns in Rust, including error handling strategies and best practices.',
					type: 'document',
				...
				},
				{
					...
					content:
						'Code examples showing proper error propagation in async contexts using ? operator and Result types.',
					type: 'search_result',
			...
			contexts: [
				{
					...
					content:
						'Performance benchmarks comparing WASM execution to native code across different workloads.',
					type: 'document',
...
test.describe('End-to-End Context Management Workflows', () => {
	...
	test('Complete workflow: Create conversation with context and messages', async ({ page }) => {
		...
		await test.step('Create new conversation', async () => {
			...
			await expect(page.locator(`[data-testid="conversation-title-${conversationId}"]`)).toHaveText(
		...
		await test.step('Add context items to conversation', async () => {
			for (const [index, context] of conversationData.contexts.entries()) {
				...
				await expect(page.locator(`[data-testid="context-title-${index}"]`)).toHaveText(
					context.title
				);

				// Verify context type indicator
				await expect(page.locator(`[data-testid="context-type-${index}"]`)).toHaveText(
		...
		await test.step('Add messages to conversation', async () => {
			for (const [index, message] of conversationData.messages.entries()) {
				...
				await expect(page.locator(`[data-testid="message-content-${index}"]`)).toContainText(
					message.content
				);

				// Verify message role styling
				if (message.role === 'user') {
					await expect(page.locator(`[data-testid="message-${index}"]`)).toHaveClass(
						/user-message/
					);
				} else {
					await expect(page.locator(`[data-testid="message-${index}"]`)).toHaveClass(
		...
		await test.step('Verify complete conversation state', async () => {
			...
			const conversationTitle = await page
				.locator('[data-testid="current-conversation-title"]')
				.textContent();
			expect(conversationTitle).toBe(conversationData.title);

			const conversationRole = await page
				.locator('[data-testid="current-conversation-role"]')
				.textContent();
	...
	test('Multi-conversation workflow with context switching', async ({ page }) => {
		...
		await test.step('Test conversation switching and state isolation', async () => {
			...
			const firstConversationContent = await page
				.locator('[data-testid="message-content-0"]')
				.textContent();
			expect(firstConversationContent).toBe(
				WORKFLOW_TEST_DATA.conversations[1].messages[0].content
			);
		});
	...
	test('Search integration workflow', async ({ page }) => {
		...
		await test.step('Verify search result context integration', async () => {
			...
			await expect(contextDetails.locator('[data-testid="context-source"]')).toContainText(
				'search'
			);

			await expect(contextDetails.locator('[data-testid="context-rank"]')).toBeVisible();
	...
	test('Context management limits and edge cases', async ({ page }) => {
		...
		await test.step('Test context length limits', async () => {
			...
			const hasTruncationWarning = await page
				.locator('[data-testid="content-truncated-warning"]')
				.isVisible();
			const hasLengthWarning = await page
				.locator('[data-testid="context-length-warning"]')
				.isVisible();
		...
		await test.step('Test context removal and cleanup', async () => {
			...
			await expect(page.locator('[data-testid="context-title-0"]')).toHaveText(
				'Removable Context 1'
			);
			await expect(page.locator('[data-testid="context-title-1"]')).toHaveText(
				'Removable Context 3'
			);
		});
	});
	...
	test('Error handling and recovery workflow', async ({ page }) => {
		...
		await test.step('Test network error handling', async () => {
			...
			await expect(page.locator('[data-testid="context-error-message"]')).toContainText(
		...
		await test.step('Test invalid data handling', async () => {
			...
			await expect(page.locator('[data-testid="title-validation-error"]')).toContainText(
				'Title is required'
			);
	...
	test('Accessibility and keyboard navigation workflow', async ({ page }) => {
		...
		await test.step('Test keyboard navigation for conversation creation', async () => {
			...
			await expect(page.locator('[data-testid="current-conversation-title"]')).toHaveText(
		...
		await test.step('Test screen reader accessibility', async () => {
			...
			await expect(page.locator('[data-testid="context-panel"]')).toHaveAttribute(
				'aria-label',
				'Context panel'
			);
