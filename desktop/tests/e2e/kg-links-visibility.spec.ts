...
test.describe('KG Links Visibility Tests', () => {
	...
	test('should display KG links in search results when using Terraphim Engineer role', async ({
		page,
	}) => {
	...
	test('should NOT display KG links when using a role without terraphim_it enabled', async ({
		page,
	}) => {
	...
	test('should maintain KG links when switching between documents', async ({ page }) => {
		...
		console.log(
			`First document KG links: ${firstDocLinkCount}, Second document KG links: ${secondDocLinkCount}`
		);
	});
