...
test.describe('Ripgrep Tag Filtering Configuration', () => {
	...
	test('should not show tag filtering for Atomic haystacks', async ({ page }) => {
		...
		await expect(
			page.locator('label:has-text("Extra Parameters (for filtering)")')
		).not.toBeVisible();
	...
	test('should save and load tag configuration correctly', async ({ page }) => {
		...
		expect(savedConfig.config.roles['Tag Test Role'].haystacks[0].extra_parameters.tag).toBe(
			'#rust'
		);
		expect(savedConfig.config.roles['Tag Test Role'].haystacks[0].extra_parameters.max_count).toBe(
			'10'
		);
