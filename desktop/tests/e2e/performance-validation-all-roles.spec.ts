...
test.describe('Performance Validation - All Roles', () => {
	...
	test('should validate Terraphim Engineer role search performance', async ({ page }) => {
		...
		const metrics = await performSearchWithTiming(
			page,
		...
		console.log(
			`✅ Terraphim Engineer role: ${metrics.searchTime}ms, ${metrics.resultCount} results`
		);
	});
	...
	test('should validate Front End Engineer role search performance', async ({ page }) => {
		...
		console.log(
			`✅ Front End Engineer role: ${metrics.searchTime}ms, ${metrics.resultCount} results`
		);
	});
	...
	test.afterAll(async () => {
		...
		for (const metrics of performanceMetrics) {
			...
			console.log(
				`${status} ${metrics.role}: ${metrics.searchTime}ms (${metrics.resultCount} results)`
			);
		}

		const avgSearchTime =
			performanceMetrics.reduce((sum, m) => sum + m.searchTime, 0) / performanceMetrics.length;
		...
		console.log(
			`  UI responsive: ${performanceMetrics.every((m) => m.uiResponsive) ? '✅' : '❌'}`
		);
	});
});
...
async function switchToRole(page: Page, roleName: string): Promise<void> {
	...
	const roleButton = page
		.locator('button:has-text("Theme"), .role-switcher, [data-testid="role-switcher"]')
		.first();
 ...
 */
async function performSearchWithTiming(
	page: Page,
	query: string,
	role: string
