...
async function startBackendServices(): Promise<TestEnvironment> {
	...
	try {
		const mcpProcess = exec(
			`./target/release/terraphim_mcp_server --sse --bind 127.0.0.1:${MCP_SERVER_PORT}`,
			{
	...
	if (!environment.backendReady) {
		throw new Error(
			`Backend server failed to start on port ${BACKEND_PORT} after ${MAX_RETRIES} attempts`
		);
	}
...
async function setupTestData(): Promise<void> {
	...
	try {
		await fs.writeFile('test-results/test-data.json', JSON.stringify(testConfig, null, 2));
...
async function globalSetup(config: FullConfig): Promise<void> {
	...
	try {
		...
		await fs.writeFile('test-results/environment.json', JSON.stringify(environmentState, null, 2));
