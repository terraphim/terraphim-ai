...
async function stopMCPServer(): Promise<void> {
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      try {
        await execAsync(`kill -0 ${pid}`);
        console.log(`⏳ Waiting for process ${pid} to terminate... (${attempts + 1}/${maxAttempts})`);
...
async function globalTeardown(config: FullConfig): Promise<void> {
  ...
  try {
    ...
    const teardownDuration = Date.now() - teardownStartTime;
    console.error(`💥 Global teardown encountered errors after ${teardownDuration}ms:`, error.message);
