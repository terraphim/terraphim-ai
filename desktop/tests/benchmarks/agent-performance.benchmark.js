...
describe('Agent Performance Benchmarks', () => {
	...
	beforeAll(async () => {
		...
		serverProcess = spawn(
			...
			[
				...
				'--port',
				BENCHMARK_CONFIG.serverPort.toString(),
			],
			{
				stdio: 'pipe',
				cwd: process.cwd() + '/..',
	...
	afterAll(async () => {
		...
		Object.values(measurements).forEach((measurement) => {
			...
			if (threshold) {
				...
				if (!avgPassed)
					console.log(
						`     ⚠️  Average ${stats.avg.toFixed(2)}ms exceeds threshold ${threshold.avg}ms`
					);
				if (!p95Passed)
					console.log(`     ⚠️  P95 ${stats.p95.toFixed(2)}ms exceeds threshold ${threshold.p95}ms`);
			}
		});
	...
	describe('WebSocket Connection Performance', () => {
		it('should establish connections within performance thresholds', async () => {
			...
			console.log(
				`📊 Connection Performance: ${BENCHMARK_CONFIG.scale.connectionLoad} connections`
			);
