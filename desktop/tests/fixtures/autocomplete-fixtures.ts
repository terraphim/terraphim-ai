...
import {
	...
	AutocompleteTestConfig,
} from '../e2e/helpers/autocomplete-helpers';
...
export const EXPECTED_SUGGESTIONS: Record<string, AutocompleteSuggestion[]> = {
	terraphim: [
		...
		{ text: 'terraphim-automata', snippet: 'Text matching and autocomplete', score: 0.9 },
	...
	graph: [
		...
		{ text: 'terraphim-graph', snippet: 'Knowledge graph implementation', score: 0.9 },
	...
	search: [
		...
		{ text: 'role-based-search', snippet: 'Personalized search by role', score: 0.9 },
...
export const MOCK_SUGGESTIONS: AutocompleteSuggestion[] = [
	...
	{ text: 'haystack-integration', snippet: 'Data source integration', score: 0.8 },
...
export const MOCK_MCP_RESPONSES = {
	...
	emptyResponse: {
		...
		id: 1,
		result: {
			content: [{ type: 'text', text: 'Found 0 suggestions' }],
		},
	},
...
export function validateSuggestionStructure(suggestion: any): boolean {
	return (
		...
		(suggestion.score === undefined ||
			(typeof suggestion.score === 'number' && suggestion.score >= 0 && suggestion.score <= 1))
	);
}
...
export function createTestScenarios(
	...
	return [
		...
		{
			query: baseQuery.charAt(0).toUpperCase() + baseQuery.slice(1),
			description: `Title case: ${baseQuery.charAt(0).toUpperCase() + baseQuery.slice(1)}`,
		},
	];
}
