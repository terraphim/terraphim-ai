...
async fn test_desktop_startup_terraphim_engineer_role_functional() {
    ...
    println!(
        "  ✅ Terraphim Engineer role found with relevance function: {:?}",
        terraphim_role.relevance_function
    );
    ...
    let search_queries = vec![
        (
            "haystack",
            "Should find haystack.md - KG term with synonyms: datasource, service, agent",
        ),
        (
            "service",
            "Should find service.md - KG term with synonyms: provider, middleware",
        ),
        (
            "terraphim-graph",
            "Should find terraphim-graph.md - KG term with graph embeddings synonyms",
        ),
        (
            "graph embeddings",
            "Should match terraphim-graph synonyms from KG",
        ),
    ...
    for (search_term, expectation) in search_queries {
        println!(
            "  🔎 Testing search with term: '{}' - {}",
            search_term, expectation
        );
    ...
    for (term, expectation) in kg_terms {
        ...
        match kg_result {
            Ok(documents) => {
                println!(
                    "    ✅ KG lookup succeeded for '{}': {} documents",
                    term,
                    documents.len()
                );
    ...
    for (name, role_config) in &config_data.roles {
        println!(
            "    - {} -> {:?}",
            name.original, role_config.relevance_function
        );
    }
    ...
    if total_results_found > 0 || total_kg_results > 0 {
        ...
        println!(
            "🎉 AWS fix is 100% successful! TerraphimGraph scoring needs separate investigation."
        );
    }
}
...
async fn test_desktop_config_loading_without_aws_errors() {
    ...
    println!(
        "    ✅ TerraphimService created successfully with {} roles",
        service_config.roles.len()
    );
