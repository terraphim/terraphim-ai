 ...
 */
function validateRipgrepTags(tagsString: string): { isValid: boolean; errors: string[]; parameters: string[] } {
 ...
 */
function validateRipgrepExtraParameters(extraParams: Record<string, string>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  for (const [key, value] of Object.entries(extraParams)) {
    switch (key) {
      ...
      case 'type':
        ...
        const validTypes = ['md', 'markdown', 'rs', 'rust', 'js', 'javascript', 'ts', 'typescript', 'py', 'python', 'txt', 'text'];
        if (!validTypes.includes(value.toLowerCase())) {
          errors.push(`type "${value}" may not be supported by ripgrep. Common types: ${validTypes.join(', ')}`);
        }
        break;
...
describe('Ripgrep Tag Validation', () => {
  ...
  describe('Extra Parameters Validation', () => {
    ...
    it('should reject invalid max_count', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.includes('max_count must be a positive integer'))).toBe(true);
    });
    
    it('should reject invalid context', () => {
      const result = validateRipgrepExtraParameters({ context: '-1' });
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.includes('context must be a non-negative integer'))).toBe(true);
    });
