...
describe('Context Edit Workflow E2E Tests', () => {
  ...
  describe('Complete Context Management Workflow', () => {
    it('should add, edit, and delete context through the UI', async () => {
      ...
      const contextItem = screen.getByText('E2E Test Document').closest('[data-testid*="context-item"]');
  ...
  describe('Real API Integration E2E', () => {
    it('should perform complete CRUD operations via real API', async () => {
      ...
      // Update context
      const updateResponse = await fetch(`${serverUrl}/conversations/${conversationId}/context/${contextId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      expect(verifyData.conversation.global_context[0].title).toBe('Updated API CRUD Test Document');
      expect(verifyData.conversation.global_context[0].summary).toBe('Updated summary after CRUD operations');

      // Delete context
      const deleteResponse = await fetch(`${serverUrl}/conversations/${conversationId}/context/${contextId}`, {
        method: 'DELETE',
      });
