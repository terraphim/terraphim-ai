		...
		try {
			...
			const contextResponse = await fetch(`${baseUrl}/conversations/${conversationId}/context`, {
				...
				body: JSON.stringify({
					...
					content:
						'This is a test document for context management integration testing. It should appear in the conversation context.',
					metadata: {
						source_type: 'test',
			...
			const foundConversation = listData.conversations.find(
