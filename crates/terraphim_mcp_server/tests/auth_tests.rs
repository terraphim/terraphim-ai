use terraphim_mcp_server::auth::{AuthManager, ApiKey};
...
async fn test_mcp_authentication_system() {
    ...
    let has_list_perm = auth_manager.check_permission("test_key_123", "tool:list").await;
    assert!(has_list_perm.is_ok());
    assert!(has_list_perm.unwrap());
    
    let has_admin_perm = auth_manager.check_permission("test_key_123", "tool:admin").await;
...
async fn test_mcp_rate_limiting() {
    ...
    let api_key = ApiKey::new("limited_key", "Limited Key")
