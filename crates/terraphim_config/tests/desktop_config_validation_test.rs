...
async fn test_desktop_config_default_role_basic() {
    ...
    println!("   - Relevance function: {:?}", default_role.relevance_function);
...
async fn test_desktop_config_terraphim_engineer_uses_local_kg() {
    ...
    assert!(local_kg.path.to_string_lossy().ends_with("kg"),
            "Local KG path should point to kg subdirectory in user's data folder, got: {:?}", local_kg.path);
...
async fn test_desktop_config_roles_consistency() {
    ...
    assert_eq!(config.roles.len(), 2, "Desktop config should have exactly 2 roles");

    // Verify default role is set correctly
    assert_eq!(config.default_role, "Terraphim Engineer".into(), 
    ...
    let kg_path = terraphim_engineer_role.kg.as_ref().unwrap()
    ...
    assert_eq!(default_haystack_path, terraphim_haystack_path,
