...
impl Store {
        ...
        if let Some(agent) = &self.config.agent {
            ...
            for (k, v) in auth_headers.iter() {
                request = request.header(
                    k.as_str(),
                    v.to_str().map_err(|e| AtomicError::Parse(e.to_string()))?,
                );
            }
        }
    ...
    pub async fn create_resource(&self, resource: Resource) -> Result<Resource> {
        ...
        if let Some(agent) = &self.config.agent {
            let auth_headers =
                crate::auth::get_authentication_headers(agent, &resource.subject, "POST")?;
            for (key, value) in auth_headers.iter() {
                request = request.header(
                    key.as_str(),
                    value
                        .to_str()
                        .map_err(|e| AtomicError::Parse(e.to_string()))?,
                );
            }
        }
    ...
    pub async fn create_with_commit(
        &self,
        subject: &str,
        properties: HashMap<String, Value>,
    ) -> Result<Value> {
        // Ensure we have an agent
        let agent = self.config.agent.as_ref().ok_or_else(|| {
            AtomicError::Authentication("No agent configured for authentication".to_string())
        })?;

        // Create a commit
        let commit =
            Commit::new_create_or_update(subject.to_string(), properties, agent)?.sign(agent)?;
    ...
    pub async fn update_with_commit(
        &self,
        subject: &str,
        properties: HashMap<String, Value>,
    ) -> Result<Value> {
    ...
    pub async fn delete_with_commit(&self, subject: &str) -> Result<Value> {
        ...
        // Create a commit
        let commit = Commit::new_delete(subject.to_string(), agent)?.sign(agent)?;
    ...
    pub async fn query(&self, collection_url: &str, params: &[(&str, &str)]) -> Result<Value> {
        ...
        if let Some(agent) = &self.config.agent {
            let auth_headers =
                crate::auth::get_authentication_headers(agent, collection_url, "POST")?;
            for (key, value) in auth_headers.iter() {
                request = request.header(
                    key.as_str(),
                    value
