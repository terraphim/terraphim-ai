...
This plan translates the requirements in `docs/specs/anthropic_tests_specs.md` into executable Rust test suites. Each section maps the Anthropic Platform surface area to concrete `cargo test` targets, identifies fixtures/mocks, and calls out validation checkpoints (headers, payload schemas, streaming, and error handling).
...
- Streaming variant: drive `ChatRequest::stream = Some(true)` through the Axum handler, assert SSE framing and final message assembly.
