...
fn test_thesaurus_response_deserialization() {
    ...
    let thesaurus = thesaurus_response.thesaurus.as_ref().unwrap();
    assert_eq!(thesaurus.len(), 2);
    assert_eq!(thesaurus["machine learning"], "artificial intelligence");
    assert_eq!(thesaurus["ML"], "machine learning");
}
