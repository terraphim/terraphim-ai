...
mod tests {
    ...
    async fn tfidf_exact_match_scores_high() {
        ...
        triggers.insert("1".to_string(), "managing dependencies in rust projects".to_string());
        index.build(triggers);

        let results = index.query("managing dependencies rust");
        assert!(!results.is_empty());
        assert_eq!(results[0].0, "1".to_string());
    ...
    async fn two_pass_aho_corasick_first() {
        ...
        let mut index = TriggerIndex::new(0.3);
        let mut triggers = AHashMap::new();
        triggers.insert("999".to_string(), "fallback trigger when no synonym match".to_string());
        index.build(triggers);
        ...
        assert!(!results.is_empty());
        assert_eq!(results[0].0, "999".to_string());
    }
    ...
    async fn two_pass_fallback_to_trigger() {
        ...
        // Load trigger index
        let mut triggers = AHashMap::new();
        triggers.insert("100".to_string(), "managing dependencies cargo rust".to_string());
    ...
    async fn serializable_roundtrip_preserves_triggers() {
        ...
        assert!(serializable.trigger_descriptions.contains_key(&"1".to_string()));
        assert!(serializable.trigger_descriptions.contains_key(&"2".to_string()));
