...
///
/// A searcher is constructed by providing it a handle to an
...
impl Searcher {
    ...
    pub fn search(&mut self, query: &Query) -> Result<SearchResults<Document>> {
        if query.is_empty() {
    ...
    fn search_exhaustive(&mut self, query: &Query) -> Result<SearchResults<Document>> {
...
impl Query {
    ...
    fn matches(&self, ent: &Document) -> bool {
    ...
    fn needs_episode(&self) -> bool {
        !self.season.is_none() || !self.episode.is_none() || !self.tvshow_id.is_none()
    }
}
...
impl<E: std::error::Error + Send + Sync + 'static, T: FromStr<Err = E>> FromStr for Range<T> {
    type Err = Error;
