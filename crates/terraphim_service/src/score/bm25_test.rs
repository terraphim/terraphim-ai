use crate::score::bm25::{BM25FScorer, BM25PlusScorer, BM25Params, FieldWeights};
use crate::score::bm25_additional::{OkapiBM25Scorer, TFIDFScorer, JaccardScorer, QueryRatioScorer};
...
fn test_scorer_case_insensitive_matching() {
    ...
    let scores_lower: Vec<f64> = documents.iter()
        .map(|doc| bm25_scorer.score(query_lower, doc))
        .collect();
    
    let scores_upper: Vec<f64> = documents.iter()
...
fn test_scorer_parameter_sensitivity() {
    ...
    let params_low_k1 = BM25Params { k1: 0.5, b: 0.75, delta: 1.0 };
    let params_high_k1 = BM25Params { k1: 2.0, b: 0.75, delta: 1.0 };
    ...
    let scores_low: Vec<f64> = documents.iter()
        .map(|doc| scorer_low.score(query, doc))
        .collect();
    
    let scores_high: Vec<f64> = documents.iter()
