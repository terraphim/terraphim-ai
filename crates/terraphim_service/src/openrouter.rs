...
impl OpenRouterService {
    ...
    pub fn new(api_key: &str, model: &str) -> Result<Self> {
        if api_key.is_empty() {
            return Err(OpenRouterError::ConfigError("API key cannot be empty".to_string()));
        }
        
        if model.is_empty() {
            return Err(OpenRouterError::ConfigError("Model name cannot be empty".to_string()));
        }
    ...
    pub async fn generate_summary(&self, content: &str, max_length: usize) -> Result<String> {
        ...
        if content.len() > MAX_CONTENT_LENGTH {
            return Err(OpenRouterError::ContentTooLong(content.len(), MAX_CONTENT_LENGTH));
        }
        ...
        log::info!("Generated summary: {} characters for model: {}", summary.len(), self.model);
        
        // Ensure the summary isn't too long
        if summary.len() > max_length + 50 { // Small buffer for natural language
    ...
    pub fn get_recommended_models() -> Vec<(&'static str, &'static str, &'static str)> {
        vec![
            ("openai/gpt-3.5-turbo", "Fast and affordable", "General purpose"),
...
mod tests {
    ...
    async fn test_service_creation() {
        ...
        #[cfg(not(feature = "openrouter"))]
        {
            ...
            assert!(matches!(result.unwrap_err(), OpenRouterError::FeatureDisabled(_)));
        }
    }
    ...
    async fn test_configuration_validation() {
        #[cfg(feature = "openrouter")]
        {
            ...
            assert!(matches!(result.unwrap_err(), OpenRouterError::ConfigError(_)));
    ...
    async fn test_content_validation() {
        #[cfg(feature = "openrouter")]
        {
            let service = OpenRouterService::new("sk-or-v1-test-key", "openai/gpt-3.5-turbo").unwrap();
            ...
            assert!(matches!(result.unwrap_err(), OpenRouterError::ContentTooLong(_, _)));
        }
    }
    ...
    async fn test_error_handling() {
        ...
        let result = service.generate_summary("This is test content for summarization.", 100).await;
        assert!(result.is_err());
        
        // The error should be either API error or HTTP error
        match result.unwrap_err() {
            OpenRouterError::ApiError(_) | OpenRouterError::HttpError(_) | OpenRouterError::RateLimited => {
