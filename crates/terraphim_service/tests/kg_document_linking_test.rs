...
async fn test_kg_search_with_running_server() {
    ...
    for query in &test_queries {
        ...
        assert!(response.status().is_success(), "Search request failed for query: '{}'", query);
        ...
        assert_eq!(search_response["status"], "success", "Search returned error status");
        
        let results = search_response["results"]
            .as_array()
            .expect("Search results should be an array");
        
        assert!(!results.is_empty(), "No search results found for query: '{}'", query);
        ...
        for (i, result) in results.iter().enumerate() {
            ...
            if body.contains("](kg:") {
                ...
                if !kg_links.is_empty() {
                    println!("   🔗 Document contains KG links: [{}](kg:...)", kg_links.join("], ["));
                }
            }
        }
        ...
        println!("✅ Query '{}' successfully returned both KG source documents and KG-linked content\n", query);
    }
