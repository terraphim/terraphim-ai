...
async fn test_service_document_persistence_integration() {
    ...
    let created_doc = service.create_document(test_document.clone())
        .await
        .expect("Failed to create document through service");

    assert_eq!(created_doc.id, test_document.id, "Created document ID should match");
    assert_eq!(created_doc.title, test_document.title, "Created document title should match");
    println!("  ✅ Document created through service: '{}'", created_doc.title);

    // Step 4: Verify document can be retrieved by ID
    println!("🔍 Step 4: Retrieving document by ID");
    let retrieved_doc = service.get_document_by_id(&test_document.id)
        .await
    ...
    assert_eq!(retrieved_doc.title, test_document.title, "Retrieved document title should match");
    assert_eq!(retrieved_doc.body, test_document.body, "Retrieved document body should match");
    assert_eq!(retrieved_doc.description, test_document.description, "Retrieved document description should match");
    assert_eq!(retrieved_doc.tags, test_document.tags, "Retrieved document tags should match");
    assert_eq!(retrieved_doc.rank, test_document.rank, "Retrieved document rank should match");
    println!("  ✅ Document retrieved successfully: '{}'", retrieved_doc.title);
    ...
    let retrieved_doc_new_instance = new_service.get_document_by_id(&test_document.id)
        .await
        .expect("Failed to retrieve document with new service instance")
        .expect("Document should exist in new service instance");

    assert_eq!(retrieved_doc_new_instance.id, test_document.id, "New instance document ID should match");
    assert_eq!(retrieved_doc_new_instance.title, test_document.title, "New instance document title should match");
    assert_eq!(retrieved_doc_new_instance.body, test_document.body, "New instance document body should match");
    ...
    let _created_special_doc = new_service.create_document(special_id_doc.clone())
        .await
        .expect("Failed to create document with special character ID");

    let retrieved_special_doc = new_service.get_document_by_id(&special_id_doc.id)
        .await
        .expect("Failed to retrieve special character document")
        .expect("Special character document should exist");

    assert_eq!(retrieved_special_doc.id, special_id_doc.id, "Special char document ID should match");
    assert_eq!(retrieved_special_doc.title, special_id_doc.title, "Special char document title should match");
...
async fn test_service_thesaurus_persistence_integration() {
    ...
    for role_name_str in challenging_role_names {
        println!("  🔍 Testing thesaurus persistence for role: '{}'", role_name_str);
        ...
        println!("    ✅ Thesaurus loaded: {} entries", thesaurus_result.len());
        ...
        println!("    ✅ Thesaurus persisted across service instances for role: '{}'", role_name_str);
    }
...
async fn test_service_search_with_persisted_data() {
    ...
    let search_results_new_instance = timeout(
        Duration::from_secs(30),
    ...
    println!("  📊 New instance search returned {} results", search_results_new_instance.len());
    ...
    assert!(!search_results_new_instance.is_empty() || search_results.is_empty(), 
...
async fn test_key_generation_consistency_in_service() {
    ...
    for (role_name_str, expected_key) in test_roles {
        ...
        println!("    ✅ Key generated correctly: '{}' → '{}'", role_name_str, actual_key);
    }
    ...
    for (doc_id, expected_key) in test_document_ids {
        ...
        println!("    ✅ Document key generated correctly: '{}' → '{}'", doc_id, actual_key);
    }
...
async fn test_unicode_persistence_in_service() {
    ...
    let created_doc = service.create_document(unicode_doc.clone())
        .await
        .expect("Failed to create unicode document");

    // Verify unicode content is preserved
    assert_eq!(created_doc.title, unicode_doc.title, "Unicode title should be preserved");
    assert_eq!(created_doc.body, unicode_doc.body, "Unicode body should be preserved");
    assert_eq!(created_doc.description, unicode_doc.description, "Unicode description should be preserved");
    assert_eq!(created_doc.tags, unicode_doc.tags, "Unicode tags should be preserved");

    // Retrieve document and verify unicode persistence
    let retrieved_doc = service.get_document_by_id(&unicode_doc.id)
        .await
        .expect("Failed to retrieve unicode document")
        .expect("Unicode document should exist");

    assert_eq!(retrieved_doc.title, unicode_doc.title, "Retrieved unicode title should match");
    assert_eq!(retrieved_doc.body, unicode_doc.body, "Retrieved unicode body should match");
    assert_eq!(retrieved_doc.description, unicode_doc.description, "Retrieved unicode description should match");
    assert_eq!(retrieved_doc.tags, unicode_doc.tags, "Retrieved unicode tags should match");
...
async fn test_large_content_persistence_in_service() {
    ...
    println!("  📄 Creating document with content size: {} bytes", large_content.len());

    // Create and verify large document
    let created_doc = service.create_document(large_doc.clone())
        .await
        .expect("Failed to create large document");

    assert_eq!(created_doc.body.len(), large_content.len(), "Large content size should be preserved");

    // Retrieve and verify large document persists correctly
    let retrieved_doc = service.get_document_by_id(&large_doc.id)
        .await
        .expect("Failed to retrieve large document")
        .expect("Large document should exist");

    assert_eq!(retrieved_doc.body.len(), large_content.len(), "Retrieved large content size should match");
    assert_eq!(retrieved_doc.body, large_content, "Retrieved large content should match exactly");

    println!("  ✅ Large content ({} bytes) persisted correctly", large_content.len());
