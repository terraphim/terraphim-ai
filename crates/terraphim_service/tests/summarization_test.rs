...
fn create_comprehensive_test_document() -> Document {
    ...
    Document {
        ...
        summarization: None,
        stub: None,
        tags: Some(vec!["architecture".to_string(), "software".to_string(), "guide".to_string()]),
        rank: Some(200),
    }
}
...
async fn test_extract_description_from_short_body() -> Result<()> {
    ...
    assert_eq!(result, "This is a short document body with sufficient content.");
...
async fn test_process_document_fields_description_extraction() -> Result<()> {
    ...
    let mut doc = create_test_document("test-desc", 
        "This document needs a description extracted from its body content for testing purposes.");
    
    let task_id = manager.process_document_fields(&mut doc, &role, true, false).await?;
...
async fn test_process_document_fields_preserves_existing_description() -> Result<()> {
    ...
    let mut doc = create_test_document("test-preserve", 
    ...
    let _task_id = manager.process_document_fields(&mut doc, &role, true, false).await?;
    
    // Should preserve existing description
    assert_eq!(doc.description.as_ref().unwrap(), "Existing description that should be preserved");
...
async fn test_process_document_fields_queues_summarization_for_long_content() -> Result<()> {
    ...
    let task_id = manager.process_document_fields(&mut doc, &role, true, true).await?;
...
async fn test_process_documents_batch() -> Result<()> {
    ...
    let mut documents = vec![
        create_test_document("batch-1", "First document in batch processing test with sufficient content."),
        create_test_document("batch-2", "Second document in batch processing test with sufficient content."),
        create_comprehensive_test_document(), // Long content
    ];
    
    let task_ids = manager.process_documents_batch(&mut documents, &role, true, true).await?;
    
    // All documents should have descriptions
    for doc in &documents {
        assert!(doc.description.is_some(), "Document {} should have description", doc.id);
    }
...
async fn test_process_documents_batch_no_description_extraction() -> Result<()> {
    ...
    let task_ids = manager.process_documents_batch(&mut documents, &role, false, false).await?;
    
    // No descriptions should be extracted
    for doc in &documents {
        assert!(doc.description.is_none(), "Document {} should not have description", doc.id);
    }
...
async fn test_process_documents_batch_mixed_content_lengths() -> Result<()> {
    ...
    assert_eq!(documents.iter().filter(|d| d.description.is_some()).count(), 3);
...
async fn test_document_fields_integration_with_persistence() -> Result<()> {
    ...
    assert!(loaded_doc.description.as_ref().unwrap().contains("Software architecture"));
    
    // Verify summarization is preserved
    assert!(loaded_doc.summarization.as_ref().unwrap().contains("comprehensive guide"));
