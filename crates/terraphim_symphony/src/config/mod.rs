...
impl ServiceConfig {
    ...
    /// Maximum retry attempts before giving up on an issue.
    ///
    /// Once an issue's retry attempt counter reaches this value it is released
    /// from the claimed set and no further retries are scheduled
    /// (TLA+ `RetryBound` invariant: `forall i: retryCount[i] <= MaxRetries`).
...
mod tests {
    ...
    #[test]
    fn max_retry_attempts_configurable() {
