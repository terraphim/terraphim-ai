...
mod tests {
    ...
    ///
    /// The test polls for up to 3 s (debounce 500 ms + processing time).
    #[test]
    fn watcher_triggers_update_on_file_write() {
        let tmp = TempDir::new().unwrap();
        let scorer = Arc::new(KgPathScorer::new(empty_thesaurus()));
        let _watcher = KgWatcher::new(tmp.path(), Arc::clone(&scorer))
            .expect("KgWatcher::new should succeed");
        ...
        // Poll: the debounce window is 500 ms; allow up to 3 s total.
        let deadline =
        ...
        loop {
            ...
            if std::time::Instant::now() >= deadline {
                panic!("KgWatcher did not reload thesaurus within 3 seconds");
            }
