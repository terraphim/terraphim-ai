...
//!
//! Imports sessions from OpenCode CLI tool stored in ~/.local/state/opencode/
...
/// OpenCode session connector
///
/// Sources sessions from OpenCode CLI stored at ~/.local/state/opencode/
...
impl SessionConnector for OpenCodeConnector {
    ...
    fn detect(&self) -> ConnectorStatus {
        if let Some(path) = self.default_path() {
            if path.exists() {
                // OpenCode stores sessions in conversation subdirectories
                // Count JSONL files across all conversation directories
    ...
    async fn import(&self, options: &ImportOptions) -> Result<Vec<Session>> {
        ...
        tracing::info!("Importing OpenCode sessions from: {}", base_path.display());

        let mut sessions = Vec::new();

        // Find all JSONL files in conversation subdirectories
        ...
        for (idx, file_path) in jsonl_files.into_iter().enumerate() {
            ...
            match self.parse_session_file(&file_path).await {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Failed to parse OpenCode session {}: {}",
                        file_path.display(),
                        e
                    );
                }
        ...
        tracing::info!(
            "Imported {} OpenCode sessions (from {} files)",
            sessions.len(),
            total
        );
        Ok(sessions)
    ...
    async fn parse_session_file(&self, path: &PathBuf) -> Result<Option<Session>> {
        ...
        for line in content.lines() {
            ...
            match serde_json::from_str::<LogEntry>(line) {
                ...
                Err(e) => {
                    tracing::trace!("Skipping malformed OpenCode line: {}", e);
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_opencode_connector_source_id() {
    ...
    #[test]
    fn test_default_path_xdg_compliance() {
        let connector = OpenCodeConnector;
        let path = connector.default_path();
        assert!(path.is_some());
        let path = path.unwrap();
        // Verify it ends with .local/state/opencode
        assert!(path.to_string_lossy().contains(".local/state/opencode"));
    }
