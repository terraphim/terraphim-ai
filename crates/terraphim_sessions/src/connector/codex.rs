//! Codex session connector
//!
//! Imports sessions from OpenAI Codex CLI tool stored in ~/.codex/
...
/// Codex session connector
///
/// Sources sessions from Codex CLI stored at ~/.codex/
...
impl SessionConnector for CodexConnector {
    ...
    fn default_path(&self) -> Option<PathBuf> {
        // Codex stores sessions in ~/.codex/
    ...
    async fn import(&self, options: &ImportOptions) -> Result<Vec<Session>> {
        ...
        tracing::info!("Importing Codex sessions from: {}", base_path.display());
        ...
        tracing::info!(
            "Imported {} Codex sessions (from {} files)",
            sessions.len(),
            total
        );
        Ok(sessions)
    ...
    async fn parse_session_file(&self, path: &PathBuf) -> Result<Option<Session>> {
        ...
        for line in content.lines() {
            ...
            match serde_json::from_str::<LogEntry>(line) {
                ...
                Err(e) => {
                    tracing::trace!("Skipping malformed Codex line: {}", e);
                }
            }
        }
        ...
        // Extract session ID from file name or first entry
        ...
        // Extract project path from first entry's working directory
...
mod tests {
    ...
    #[test]
    fn test_codex_connector_source_id() {
        let connector = CodexConnector;
        assert_eq!(connector.source_id(), "codex");
        assert_eq!(connector.display_name(), "Codex");
    }
    ...
    fn test_default_path() {
        ...
        // Verify it ends with .codex
        assert!(path.to_string_lossy().ends_with(".codex"));
    }
    ...
    async fn test_parse_session_from_jsonl_file() {
        ...
        let content = [
            r#"{"sessionId":"sess1","cwd":"/project","timestamp":"2024-01-15T10:00:00Z","message":{"role":"user","content":"How do I use async?"}}"#,
            r#"{"sessionId":"sess1","cwd":"/project","timestamp":"2024-01-15T10:00:05Z","message":{"role":"assistant","content":[{"type":"text","text":"Here is how you use async..."}]}}"#,
        ].join("\n");
        ...
        assert_eq!(session.messages[0].content, "How do I use async?");
