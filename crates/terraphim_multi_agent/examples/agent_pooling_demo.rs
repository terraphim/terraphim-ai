...
async fn demo_basic_pool_usage(
    ...
    println!(
        "📈 Updated stats: {} pools, {} agents",
        stats.total_pools, stats.total_agents
    );
    println!(
        "🎯 Pool hits: {}, misses: {}",
        stats.total_pool_hits, stats.total_pool_misses
    );
    ...
    println!(
        "✅ Command completed in {:?} (faster due to pool reuse)",
        duration
    );
    ...
    println!(
        "📊 Final stats: {} operations, avg time: {:.2}ms",
    ...
    for i in 0..5 {
        ...
        let task = tokio::spawn(async move {
            let command = CommandInput {
                ...
                text: format!(
                    "Analyze the performance characteristics of operation #{}",
                    i + 1
                ),
                metadata: std::collections::HashMap::new(),
            };
...
async fn demo_pool_management(
    ...
    for pool_info in &pools {
        ...
        println!(
            "    📅 Created: {}",
            pool_info.created_at.format("%H:%M:%S")
        );
        println!(
            "    🕐 Last used: {}",
            pool_info.last_used.format("%H:%M:%S")
        );
        println!(
            "    📊 Operations: {}",
            pool_info.stats.total_operations_processed
        );
        println!("    🎯 Current size: {}", pool_info.stats.current_pool_size);
        println!(
            "    💼 Busy agents: {}",
            pool_info.stats.current_busy_agents
        );
        println!(
            "    ⚡ Avg time: {:.2}ms",
            pool_info.stats.average_operation_time_ms
        );
    }
    ...
    println!(
        "  ⏱️  Average operation time: {:.2}ms",
        global_stats.average_operation_time_ms
    );
    println!(
        "  🎯 Pool hit rate: {:.1}%",
        (global_stats.total_pool_hits as f64
            / (global_stats.total_pool_hits + global_stats.total_pool_misses) as f64)
            * 100.0
    );
...
async fn demo_multiple_roles(pool_manager: &PoolManager) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let roles = vec![
        ...
        (
            &research_role,
            "Research the latest trends in AI agent systems",
        ),
        (
            &documentation_role,
            "Document the agent pooling system architecture",
        ),
    ];
