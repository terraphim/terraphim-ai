...
fn test_sanitization_performance_normal_prompt() {
    ...
    assert!(duration < Duration::from_millis(100), 
            "Sanitization too slow: {:?}", duration);
}
...
fn test_sanitization_performance_malicious_prompt() {
    ...
    assert!(duration < Duration::from_millis(150), 
            "Malicious sanitization too slow: {:?}", duration);
}
...
fn test_max_length_enforcement() {
    ...
    assert!(!result_10k.was_modified, "10K exactly should not be truncated");
...
fn test_many_patterns_in_single_prompt() {
    ...
    assert!(result.warnings.len() >= 4, "Should detect multiple patterns");
    assert!(duration < Duration::from_millis(100), "Should be fast even with many patterns");
}
...
fn test_regex_catastrophic_backtracking_prevention() {
    ...
    assert!(duration < Duration::from_millis(50), 
            "Possible regex backtracking issue: {:?}", duration);
}
...
fn test_unicode_handling_performance() {
    ...
    assert!(duration < Duration::from_millis(50), 
            "Unicode filtering too slow: {:?}", duration);
}
...
fn test_control_char_removal_performance() {
    ...
    assert!(duration < Duration::from_millis(50), 
            "Control char removal too slow: {:?}", duration);
}
...
fn test_no_memory_amplification() {
    ...
    assert!(result.content.len() <= prompt.len() + 100, 
