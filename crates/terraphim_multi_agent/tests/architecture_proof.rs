...
async fn test_queue_based_architecture_proof() {
    ...
    assert_eq!(role.extra.get("llm_provider").unwrap(), &serde_json::json!("openai"));
    assert_eq!(role.extra.get("openai_model").unwrap(), &serde_json::json!("gpt-3.5-turbo"));
    ...
    *agent_ref2.last_active.write().await = Utc::now();
    ...
    println!("✅ Concurrent access working - {} workers completed", results.len());
