...
fn test_api_client_construction() {
    ...
    assert_eq!(std::mem::size_of_val(&client), std::mem::size_of_val(&cloned_client));
}
...
fn test_config_response_deserialization() {
    ...
    assert!(response.is_ok(), "ConfigResponse should be deserializable: {:?}", 
    ...
    assert!(config_response.config.roles.contains_key(&RoleName::new("Default")));
}
...
fn test_summarize_request_serialization() {
    ...
    assert!(json_result.is_ok(), "SummarizeRequest should be serializable");
...
fn test_thesaurus_response_deserialization() {
    ...
    assert!(response.is_ok(), "ThesaurusResponse should be deserializable");
...
fn test_autocomplete_response_deserialization() {
    ...
    assert!(response.is_ok(), "AutocompleteResponse should be deserializable");
...
fn test_rolegraph_response_deserialization() {
    ...
    assert!(response.is_ok(), "RoleGraphResponseDto should be deserializable");
...
fn test_task_status_response_deserialization() {
    ...
    for (json_response, expected_state) in test_cases {
        ...
        assert!(response.is_ok(), "TaskStatusResponse should be deserializable for state {}", expected_state);
...
fn test_queue_stats_response_deserialization() {
    ...
    assert!(response.is_ok(), "QueueStatsResponse should be deserializable");
    ...
    let sum = stats_response.pending_tasks + stats_response.processing_tasks + 
              stats_response.completed_tasks + stats_response.failed_tasks;
...
fn test_batch_summarize_request_serialization() {
    ...
    assert!(json_result.is_ok(), "BatchSummarizeRequest should be serializable");
...
fn test_boundary_values() {
    ...
    assert!(minimal_doc_json.is_ok(), "Should serialize minimal document");
}
