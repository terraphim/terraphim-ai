...
async fn start_test_server() -> Result<(Child, String)> {
    ...
    for attempt in 1..=30 {
        ...
        match server.try_wait() {
            Ok(Some(status)) => {
                return Err(anyhow::anyhow!("Server exited early with status: {}", status));
            }
            Ok(None) => {},
    ...
    let _ = server.kill();
    Err(anyhow::anyhow!("Server failed to become ready within 30 seconds"))
}
...
async fn test_end_to_end_offline_workflow() -> Result<()> {
    ...
    println!("✓ Initial config loaded: id={}, selected_role={}", 
    ...
    println!("✓ Available roles: {}", if roles.is_empty() { "(none)" } else { &roles });
    
    // 3. Set a custom role
    let custom_role = "E2ETestRole";
    let (set_stdout, _, set_code) = run_offline_command(&["config", "set", "selected_role", custom_role])?;
    assert_eq!(set_code, 0, "Setting role should succeed");
    assert!(extract_clean_output(&set_stdout).contains(&format!("updated selected_role to {}", custom_role)));
    ...
    let (_search_stdout, _, search_code) = run_offline_command(&["search", "integration test", "--limit", "3"])?;
    assert!(search_code == 0 || search_code == 1, "Search should complete");
    println!("✓ Search with custom role completed: {}", if search_code == 0 { "success" } else { "no results" });
    ...
    println!("✓ Graph command output: {} lines", graph_output.lines().count());
    ...
    let (_extract_stdout, _, extract_code) = run_offline_command(&["extract", test_text, "--exclude-term"])?;
    assert!(extract_code == 0 || extract_code == 1, "Extract should complete");
    println!("✓ Extract command completed: {}", if extract_code == 0 { "success" } else { "no matches" });
...
async fn test_end_to_end_server_workflow() -> Result<()> {
    ...
    println!("✓ Server config loaded: id={}, selected_role={}", 
    ...
    assert!(!server_roles.is_empty(), "Server should have roles available");
    
    // 3. Test search with server
    let (_search_stdout, _, search_code) = run_server_command(&server_url, &["search", "integration test", "--limit", "3"])?;
    ...
    if server_roles.len() > 1 {
        ...
        let (_search_role_stdout, _, search_role_code) = run_server_command(&server_url, 
            &["search", "test", "--role", test_role, "--limit", "2"])?;
        assert!(search_role_code == 0 || search_role_code == 1, "Server search with role should complete");
        println!("✓ Server search with role override '{}' completed", test_role);
    }
    
    // 5. Test graph with server
    let (_graph_stdout, _, graph_code) = run_server_command(&server_url, &["graph", "--top-k", "5"])?;
    assert_eq!(graph_code, 0, "Server graph should succeed");
    println!("✓ Server graph command completed");
    
    // 6. Test chat with server
    let (_chat_stdout, _, chat_code) = run_server_command(&server_url, &["chat", "Hello server test"])?;
    ...
    let (_extract_stdout, _, extract_code) = run_server_command(&server_url, &["extract", test_text])?;
    ...
    let (set_stdout, _, set_code) = run_server_command(&server_url, 
        &["config", "set", "selected_role", "Terraphim Engineer"])?;
    assert_eq!(set_code, 0, "Server config set should succeed");
    assert!(extract_clean_output(&set_stdout).contains("updated selected_role to Terraphim Engineer"));
...
async fn test_offline_vs_server_mode_comparison() -> Result<()> {
    ...
    for cmd_args in test_commands {
        ...
        // Run in server mode
        let (server_stdout, _server_stderr, server_code) = run_server_command(&server_url, &cmd_args)?;
        
        println!("  Offline: code={}, Server: code={}", offline_code, server_code);
        ...
        if cmd_args[0] == "roles" && cmd_args[1] == "list" {
            ...
            println!("    ✓ Configs have correct IDs: Offline={}, Server={}", 
                     offline_config["id"], server_config["id"]);
        } else {
            // Other commands should generally succeed in both modes
            assert!(offline_code == 0 || offline_code == 1, "Offline command should complete");
            assert!(server_code == 0 || server_code == 1, "Server command should complete");
        }
    }
...
async fn test_role_consistency_across_commands() -> Result<()> {
    ...
    for (cmd_name, cmd_args) in commands {
        ...
        assert!(code == 0 || code == 1, "Command '{}' should complete: stderr={}", cmd_name, stderr);
        ...
        if cmd_name == "chat" && code == 0 {
            ...
            assert!(output.contains(test_role) || output.contains("No LLM configured"),
                   "Chat should use selected role '{}': {}", test_role, output);
        }
    ...
    for (cmd_name, cmd_args) in vec![
        ("search", vec!["search", "test", "--role", override_role, "--limit", "1"]),
        ("graph", vec!["graph", "--role", override_role, "--top-k", "2"]),
        ("chat", vec!["chat", "override test", "--role", override_role]),
        ("extract", vec!["extract", "test text", "--role", override_role]),
    ] {
        let (stdout, _stderr, code) = run_offline_command(&cmd_args)?;
        
        assert!(code == 0 || code == 1, "Command '{}' with role override should complete", cmd_name);
        
        if cmd_name == "chat" && code == 0 {
            let output = extract_clean_output(&stdout);
            assert!(output.contains(override_role) || output.contains("No LLM configured"),
                   "Chat should use override role '{}': {}", override_role, output);
        }
...
async fn test_full_feature_matrix() -> Result<()> {
    ...
    for (mode_name, _) in modes {
        ...
        for (test_name, args) in basic_tests {
            ...
            assert_eq!(code, 0, "Basic test '{}' should succeed in {} mode: stderr={}", 
        ...
        let advanced_tests = vec![
            ...
            ("search-with-role", vec!["search", "test", "--role", "Default", "--limit", "2"]),
            ("graph-default", vec!["graph", "--top-k", "3"]),
            ("graph-with-role", vec!["graph", "--role", "Default", "--top-k", "3"]),
            ("chat-default", vec!["chat", "test message"]),
            ("chat-with-role", vec!["chat", "test message", "--role", "Default"]),
            ("extract-default", vec!["extract", "test text for extraction"]),
            ("extract-with-options", vec!["extract", "test text", "--role", "Default", "--exclude-term"]),
        ];
        
        for (test_name, args) in advanced_tests {
            let (_stdout, stderr, code) = run_offline_command(&args)?;
            assert!(code == 0 || code == 1, "Advanced test '{}' should complete in {} mode: stderr={}", 
                   test_name, mode_name, stderr);
        ...
        let config_tests = vec![
            ("config-set-role", vec!["config", "set", "selected_role", "TestRole"]),
        ];
        
        for (test_name, args) in config_tests {
            let (_stdout, stderr, code) = run_offline_command(&args)?;
            assert_eq!(code, 0, "Config test '{}' should succeed in {} mode: stderr={}", 
                      test_name, mode_name, stderr);
    ...
    if let Some((mut server, server_url)) = server_info {
        ...
        for (test_name, args) in server_tests {
            ...
            assert_eq!(code, 0, "Server test '{}' should succeed: stderr={}", test_name, stderr);
            println!("  ✓ {}: succeeded", test_name);
        }
