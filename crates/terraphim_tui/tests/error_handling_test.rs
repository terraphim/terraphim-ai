...
async fn is_server_running() -> bool {
    ...
    timeout(Duration::from_secs(2), client.health()).await
    ...
    match result {
        ...
        Ok(Err(e)) => {
            ...
            assert!(
                ...
                "Error should indicate network issue: {}", e
            );
        }
...
async fn test_malformed_server_response() {
    ...
    match result {
        Ok(response) => {
            ...
            assert!(response.results.len() <= 10000, "Results should be capped to reasonable size");
        }
        Err(e) => {
            ...
            assert!(
                ...
                "Should not be internal server error: {}", e
            );
        }
    }
}
...
async fn test_invalid_role_handling() {
    ...
    match result {
        Ok(response) => {
            println!("✅ Invalid role handled with response: status={}, results={}", 
                    response.status, response.results.len());
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "Should be role-related error: {}", e
            );
        }
    }
}
...
async fn test_empty_and_special_character_queries() {
    ...
    for (i, query) in special_queries.iter().enumerate() {
        ...
        match result {
            Ok(response) => {
                println!("✅ Special query {} handled: status={}, results={}", 
                        i, response.status, response.results.len());
            ...
            Err(e) => {
                ...
                assert!(
                    ...
                    "Should not be internal server error for query {}: {}", i, e
                );
            }
        }
    }
...
async fn test_concurrent_request_handling() {
    ...
    for (i, result) in results.into_iter().enumerate() {
        match result {
            Ok(Ok(response)) => {
                successes += 1;
                println!("✅ Concurrent request {} succeeded: status={}", i, response.status);
                assert!(!response.status.is_empty());
            }
            Ok(Err(e)) => {
                errors += 1;
                ...
                assert!(
                    !error_str.contains("panic") &&
                    !error_str.contains("internal server error"),
                    "Should not be internal error: {}", e
                );
            }
            Err(e) => {
                errors += 1;
    ...
    println!("Concurrent requests: {} successes, {} errors", successes, errors);
...
async fn test_summarization_error_handling() {
    ...
    let result = client.summarize_document(&invalid_doc, Some("Default")).await;
    
    match result {
        Ok(response) => {
            println!("✅ Invalid document summarization handled: status={}", response.status);
            // Should handle gracefully
            assert!(!response.status.is_empty());
            if response.status == "Error" {
                assert!(response.error.is_some(), "Error status should have error message");
            }
        }
        Err(e) => {
...
async fn test_autocomplete_error_handling() {
    ...
    for (role, query) in test_cases {
        ...
        match result {
            Ok(response) => {
                println!("✅ Autocomplete handled: role={}, query_len={}, status={}", 
                        role, query.len(), response.status);
                assert!(!response.status.is_empty());
            }
            Err(e) => {
                println!("✅ Autocomplete error handled: role={}, query_len={}, error={}", 
                        role, query.len(), e);
                let error_str = e.to_string().to_lowercase();
                assert!(
                    !error_str.contains("internal server error") &&
...
async fn test_client_timeout_configuration() {
    ...
    assert!(duration < Duration::from_secs(12), "Should timeout before 12 seconds");
    ...
    println!("✅ Client timeout configuration working correctly: {:?}", duration);
}
...
async fn test_graceful_degradation() {
    ...
    for (op_type, description) in operations {
        let result = match op_type {
            "config" => {
                client.get_config().await.map(|_| "Success".to_string()).map_err(|e| e.to_string())
            }
            "search" => {
                let query = SearchQuery {
                ...
                client.search(&query).await.map(|r| r.status).map_err(|e| e.to_string())
            }
            "rolegraph" => {
                client.get_rolegraph_edges(Some("Default")).await.map(|r| r.status).map_err(|e| e.to_string())
            }
    ...
    println!("Operation sequence complete. All operations succeeded: {}", all_succeeded);
}