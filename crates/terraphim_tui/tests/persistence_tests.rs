...
async fn test_persistence_setup_and_cleanup() -> Result<()> {
    ...
    assert_eq!(code, 0, "Config show should succeed and initialize persistence, stderr: {}", stderr);
    ...
    for dir in expected_dirs {
        assert!(Path::new(dir).exists(), "Persistence directory should be created: {}", dir);
    ...
    assert!(Path::new(db_file).exists(), "SQLite database should be created: {}", db_file);
...
async fn test_config_persistence_across_runs() -> Result<()> {
    ...
    assert_eq!(code1, 0, "First config set should succeed, stderr: {}", stderr1);
    assert!(extract_clean_output(&stdout1).contains(&format!("updated selected_role to {}", test_role)),
...
async fn test_role_switching_persistence() -> Result<()> {
    ...
    for (i, role) in roles_to_test.iter().enumerate() {
        ...
        assert_eq!(set_code, 0, "Should be able to set role '{}', stderr: {}", role, set_stderr);
        assert!(extract_clean_output(&set_stdout).contains(&format!("updated selected_role to {}", role)),
               "Should confirm role update to '{}'", role);
        
        // Verify immediately
        let (show_stdout, show_stderr, show_code) = run_tui_command(&["config", "show"])?;
        assert_eq!(show_code, 0, "Config show should work, stderr: {}", show_stderr);
        ...
        assert_eq!(current_role, *role, 
                  "Role should be set immediately: expected '{}', got '{}'", role, current_role);
    ...
    assert_eq!(final_code, 0, "Final config show should work, stderr: {}", final_stderr);
    ...
    println!("✓ All role switches persisted correctly, final role: '{}'", final_role);
...
async fn test_persistence_backend_functionality() -> Result<()> {
    ...
    for (key, value) in config_changes {
        ...
        assert_eq!(code, 0, "Config set '{}' = '{}' should succeed, stderr: {}", key, value, stderr);
    ...
    assert!(Path::new(dashmap_dir).exists(), "Dashmap directory should exist");
...
async fn test_concurrent_persistence_operations() -> Result<()> {
        ...
        match result {
            Ok((_stdout, stderr, code)) => {
                if code == 0 {
                    ...
                    println!("⚠ Concurrent operation {} (role '{}') failed: {}", i, role, stderr);
                }
            }
            Err(e) => {
    ...
    // Check final state
    let (final_stdout, final_stderr, final_code) = run_tui_command(&["config", "show"])?;
    assert_eq!(final_code, 0, "Final config check should work, stderr: {}", final_stderr);
    ...
    assert!(final_role.starts_with("ConcurrentRole"), 
           "Final role should be one of the concurrent roles: '{}'", final_role);
...
async fn test_persistence_recovery_after_corruption() -> Result<()> {
    ...
    let (_, stderr1, code1) = run_tui_command(&["config", "set", "selected_role", "PreCorruption"])?;
    assert_eq!(code1, 0, "Initial setup should succeed, stderr: {}", stderr1);
    ...
    assert_eq!(code, 0, "TUI should recover after corruption, stderr: {}", stderr);
    
    // Should create new persistence and use defaults
    let config = parse_config_from_output(&stdout)?;
    println!("✓ TUI recovered with config: id={}, selected_role={}", 
             config["id"], config["selected_role"]);
    
    // Persistence directories should be recreated
    assert!(Path::new("/tmp/terraphim_sqlite").exists(), "SQLite dir should be recreated");
    assert!(Path::new("/tmp/dashmaptest").exists(), "Dashmap dir should be recreated");
    
    // Should be able to set new values
    let (_, stderr2, code2) = run_tui_command(&["config", "set", "selected_role", "PostRecovery"])?;
    assert_eq!(code2, 0, "Should be able to set config after recovery, stderr: {}", stderr2);
...
async fn test_persistence_with_special_characters() -> Result<()> {
    ...
    for role in special_roles {
        ...
        assert_eq!(set_code, 0, "Should handle special characters in role '{}', stderr: {}", role, set_stderr);
        
        // Verify it persisted correctly
        let (show_stdout, show_stderr, show_code) = run_tui_command(&["config", "show"])?;
        assert_eq!(show_code, 0, "Config show should work with special role, stderr: {}", show_stderr);
        
        let config = parse_config_from_output(&show_stdout)?;
        let stored_role = config["selected_role"].as_str().unwrap();
        
        assert_eq!(stored_role, role, "Special character role should persist correctly");
...
async fn test_persistence_directory_permissions() -> Result<()> {
    ...
    assert_eq!(code, 0, "TUI should create directories successfully, stderr: {}", stderr);
    ...
    for dir in test_dirs {
        ...
        assert!(test_file.exists(), "Should be able to write to directory: {}", dir);
...
async fn test_persistence_backend_selection() -> Result<()> {
    ...
    let (_stdout, stderr, code) = run_tui_command(&["config", "set", "selected_role", "BackendSelectionTest"])?;
    ...
    assert_eq!(verify_code, 0, "Config show should work, stderr: {}", verify_stderr);
    
    let config = parse_config_from_output(&verify_stdout)?;
    assert_eq!(config["selected_role"], "BackendSelectionTest", "Data should persist correctly");
