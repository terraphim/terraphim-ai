...
async fn test_offline_help_command() -> Result<()> {
    ...
    assert!(stdout.contains("Terraphim TUI interface"), "Should show main help");
    assert!(stdout.contains("--server"), "Should show server flag");
    assert!(stdout.contains("--server-url"), "Should show server URL flag");
    ...
    assert!(stdout.contains("interactive"), "Should list interactive command");
...
async fn test_offline_config_show() -> Result<()> {
    ...
    assert!(config.get("selected_role").is_some(), "Should have selected_role");
    assert!(config.get("default_role").is_some(), "Should have default_role");
    assert!(config.get("global_shortcut").is_some(), "Should have global_shortcut");
...
async fn test_offline_search_with_default_role() -> Result<()> {
    ...
    assert!(code == 0 || code == 1, "Search should not crash, stderr: {}", stderr);
...
async fn test_offline_search_with_role_override() -> Result<()> {
    let (stdout, stderr, code) = run_offline_command(&["search", "test query", "--role", "Default"])?;
    
    // Search may succeed or fail depending on setup, but should not crash
    assert!(code == 0 || code == 1, "Search with role override should not crash, stderr: {}", stderr);
...
async fn test_offline_graph_command() -> Result<()> {
    ...
    let lines: Vec<&str> = stdout.lines().filter(|line| !line.contains("INFO") && !line.contains("WARN")).collect();
...
async fn test_offline_graph_with_role() -> Result<()> {
    let (stdout, stderr, code) = run_offline_command(&["graph", "--role", "Default", "--top-k", "5"])?;
    
    // Graph command with role should succeed
    assert_eq!(code, 0, "Graph command with role should succeed, stderr: {}", stderr);
...
async fn test_offline_chat_command() -> Result<()> {
    ...
    let output_lines: Vec<&str> = stdout.lines().filter(|line| !line.contains("INFO") && !line.contains("WARN")).collect();
    let response = output_lines.join("\n");
    assert!(response.contains("No LLM configured") || response.contains("Chat response"), 
           "Should show LLM response or no LLM message: {}", response);
...
async fn test_offline_extract_command() -> Result<()> {
    let text = "This is a test paragraph. It contains some text for extraction. Another sentence here.";
    let (stdout, stderr, code) = run_offline_command(&["extract", text])?;
    
    // Extract might succeed or fail based on thesaurus availability
    assert!(code == 0 || code == 1, "Extract command should not crash, stderr: {}", stderr);
...
async fn test_offline_extract_with_role() -> Result<()> {
    ...
    let (stdout, stderr, code) = run_offline_command(&["extract", text, "--role", "Default", "--exclude-term"])?;
    
    // Extract with role might succeed or fail
    assert!(code == 0 || code == 1, "Extract with role should not crash, stderr: {}", stderr);
...
async fn test_offline_config_set_selected_role() -> Result<()> {
    ...
    // Set selected role
    let (stdout, stderr, code) = run_offline_command(&["config", "set", "selected_role", "Default"])?;
    
    assert_eq!(code, 0, "Config set should succeed, stderr: {}", stderr);
    
    let output_lines: Vec<&str> = stdout.lines().filter(|line| !line.contains("INFO") && !line.contains("WARN")).collect();
    let response = output_lines.join("\n");
    assert!(response.contains("updated selected_role to Default"), 
           "Should confirm role update: {}", response);
...
async fn test_offline_roles_select() -> Result<()> {
    ...
    if code == 0 {
        let output_lines: Vec<&str> = stdout.lines().filter(|line| !line.contains("INFO") && !line.contains("WARN")).collect();
        let response = output_lines.join("\n");
        assert!(response.contains("selected:Default"), "Should confirm role selection: {}", response);
    } else {
...
async fn test_server_mode_connection_failure() -> Result<()> {
    ...
    assert!(stderr.contains("Connection refused") || stderr.contains("connect error"), 
           "Should show connection error: {}", stderr);
...
async fn test_server_mode_with_custom_url() -> Result<()> {
    ...
    let mut cmd = Command::new("cargo");
    cmd.args(&["run", "-p", "terraphim_tui", "--"])
        .args(&["--server", "--server-url", "http://localhost:9999", "config", "show"]);
    
    let output = cmd.output()?;
    let stderr = String::from_utf8_lossy(&output.stderr);
    let code = output.status.code().unwrap_or(-1);
    
    assert_eq!(code, 1, "Should fail with custom URL when no server running");
    assert!(stderr.contains("Connection refused") || stderr.contains("connect error"), 
           "Should show connection error with custom URL: {}", stderr);
...
async fn test_offline_mode_is_default() -> Result<()> {
    ...
    assert!(stderr.contains("Initializing TUI service with embedded configuration") || 
            stderr.contains("Failed to load config") || 
            stderr.contains("using default embedded"), 
           "Should show embedded initialization: {}", stderr);
