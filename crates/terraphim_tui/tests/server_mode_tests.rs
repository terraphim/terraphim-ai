...
async fn test_server_mode_config_show() -> Result<()> {
    ...
    assert_eq!(code, 0, "Server mode config show should succeed, stderr: {}", stderr);
    ...
    assert_eq!(config["selected_role"], "Terraphim Engineer", "Should have Terraphim Engineer as selected role");
...
async fn test_server_mode_roles_list() -> Result<()> {
    ...
    assert_eq!(code, 0, "Server mode roles list should succeed, stderr: {}", stderr);
    ...
    assert!(roles.iter().any(|r| r.contains("Terraphim Engineer") || r.contains("Engineer")), 
           "Should have Terraphim Engineer role: {:?}", roles);
...
async fn test_server_mode_search_with_selected_role() -> Result<()> {
    ...
    let (stdout, stderr, code) = run_server_command(&server_url, &["search", "rust programming", "--limit", "5"])?;
    
    // Cleanup
    let _ = server.kill();
    let _ = server.wait();
    
    assert_eq!(code, 0, "Server mode search should succeed, stderr: {}", stderr);
    
    println!("Search results: {}", stdout);
    
    // Should have some results or at least not error
    // Results format: "- <rank>\t<title>"
    let result_lines: Vec<&str> = stdout.lines().filter(|line| line.starts_with("- ")).collect();
...
async fn test_server_mode_search_with_role_override() -> Result<()> {
    ...
    let (stdout, stderr, code) = run_server_command(&server_url, &["search", "test", "--role", "Default", "--limit", "3"])?;
    ...
    if code == 0 {
        ...
        println!("Search with role override failed (role may not exist): {}", stderr);
    }
...
async fn test_server_mode_roles_select() -> Result<()> {
    ...
    assert_eq!(code, 0, "Server mode role select should succeed, stderr: {}", stderr);
    assert!(stdout.contains(&format!("selected:{}", first_role)), 
           "Should confirm role selection: {}", stdout);
...
async fn test_server_mode_graph_command() -> Result<()> {
    ...
    assert_eq!(code, 0, "Server mode graph should succeed, stderr: {}", stderr);
...
async fn test_server_mode_chat_command() -> Result<()> {
    ...
    assert_eq!(code, 0, "Server mode chat should succeed, stderr: {}", stderr);
...
async fn test_server_mode_extract_command() -> Result<()> {
    ...
    assert_eq!(code, 0, "Server mode extract should succeed, stderr: {}", stderr);
    
    println!("Extract results: {}", stdout);
    
    // Should either find matches or report no matches
    assert!(stdout.contains("Found") || stdout.contains("No matches"), 
           "Should report extract results: {}", stdout);
...
async fn test_server_mode_config_set() -> Result<()> {
    ...
    let (stdout, stderr, code) = run_server_command(&server_url, &["config", "set", "selected_role", "Terraphim Engineer"])?;
    
    // Cleanup server first
    let _ = server.kill();
    let _ = server.wait();
    
    assert_eq!(code, 0, "Server mode config set should succeed, stderr: {}", stderr);
    assert!(stdout.contains("updated selected_role to Terraphim Engineer"), 
           "Should confirm config update: {}", stdout);
...
async fn test_server_vs_offline_mode_comparison() -> Result<()> {
    ...
    let (server_stdout, _server_stderr, server_code) = run_server_command(&server_url, &["config", "show"])?;
    ...
    assert_eq!(server_config["id"], "Server", "Server should use Server config");
    assert_eq!(offline_config["id"], "Embedded", "Offline should use Embedded config");
...
async fn test_server_startup_and_health() -> Result<()> {
    ...
    assert!(response.status().is_success(), "Server health check should pass");
