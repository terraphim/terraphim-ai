...
impl ApiClient {
    pub fn new(base_url: impl Into<String>) -> Self {
        Self { http: Client::new(), base: base_url.into() }
    }

    pub async fn health(&self) -> Result<()> {
        let url = format!("{}/health", self.base);
        let res = self.http.get(url).send().await?;
        if res.status().is_success() { Ok(()) } else { anyhow::bail!("health check failed") }
    }

    pub async fn get_config(&self) -> Result<ConfigResponse> {
        let url = format!("{}/config", self.base);
        let res = self.http.get(url).send().await?;
        let body = res.error_for_status()?.
            json::<ConfigResponse>().await?;
        Ok(body)
    }
    ...
    pub async fn update_selected_role(&self, role: &str) -> Result<ConfigResponse> {
        ...
        #[derive(Serialize)]
        struct Payload<'a> { selected_role: &'a str }
        let res = self.http.post(url).json(&Payload { selected_role: role }).send().await?;
    ...
    pub async fn rolegraph(&self, role: Option<&str>) -> Result<RoleGraphResponseDto> {
        let url = match role {
            Some(r) if !r.is_empty() => format!("{}/rolegraph?role={}", self.base, urlencoding::encode(r)),
            _ => format!("{}/rolegraph", self.base),
        };
        let res = self.http.get(url).send().await?;
        let body = res.error_for_status()?.json::<RoleGraphResponseDto>().await?;
        Ok(body)
    }
}
...
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct GraphNodeDto { pub id: u64, pub label: String, pub rank: u64 }

#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct GraphEdgeDto { pub source: u64, pub target: u64, pub rank: u64 }
...
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct ChatRequest { pub role: String, pub messages: Vec<ChatMessage>, pub model: Option<String> }

#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct ChatResponse { pub status: String, pub message: Option<String>, pub model_used: Option<String>, pub error: Option<String> }

impl ApiClient {
    pub async fn chat(&self, role: &str, user_message: &str, model: Option<&str>) -> Result<ChatResponse> {
        let url = format!("{}/chat", self.base);
        let req = ChatRequest { role: role.to_string(), messages: vec![ChatMessage{ role: "user".into(), content: user_message.into() }], model: model.map(|s| s.to_string()) };
