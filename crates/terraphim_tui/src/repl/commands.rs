...
impl FromStr for ReplCommand {
    ...
    fn from_str(input: &str) -> Result<Self> {
        ...
        match parts[0] {
            ...
            "config" => {
                if parts.len() < 2 {
                    return Err(anyhow!("Config command requires a subcommand (show | set <key> <value>)"));
                }
            ...
            "role" => {
                if parts.len() < 2 {
                    return Err(anyhow!("Role command requires a subcommand (list | select <name>)"));
                }
            ...
            "graph" => {
                ...
                while i < parts.len() {
                    match parts[i] {
                        "--top-k" => {
                            if i + 1 < parts.len() {
                                top_k = Some(parts[i + 1].parse::<usize>()
            ...
            "summarize" => {
                if parts.len() < 2 {
                    return Err(anyhow!("Summarize command requires a target (document ID or text)"));
                }
...
impl ReplCommand {
    ...
    pub fn available_commands() -> Vec<&'static str> {
        ...
        #[cfg(feature = "repl-mcp")]
        {
            commands.extend_from_slice(&["autocomplete", "extract", "find", "replace", "thesaurus"]);
        }
...
mod tests {
    ...
    fn test_search_command_parsing() {
        ...
        assert_eq!(cmd, ReplCommand::Search { 
            query: "hello world".to_string(), 
            role: None, 
            limit: None 
        });

        let cmd = "/search test --role Engineer --limit 5".parse::<ReplCommand>().unwrap();
        assert_eq!(cmd, ReplCommand::Search { 
    ...
    fn test_config_command_parsing() {
        ...
        assert_eq!(cmd, ReplCommand::Config { 
            subcommand: ConfigSubcommand::Show 
        });

        let cmd = "/config set selected_role Engineer".parse::<ReplCommand>().unwrap();
        assert_eq!(cmd, ReplCommand::Config { 
            subcommand: ConfigSubcommand::Set {
    ...
    fn test_utility_commands() {
        ...
        assert_eq!(help_cmd, ReplCommand::Help { 
