...
/// Formats a [`ProviderUsage`] snapshot as a human-readable plain-text string.
...
/// Serialises a [`ProviderUsage`] snapshot to a pretty-printed JSON string.
pub fn format_usage_json(usage: &ProviderUsage) -> Result<String, serde_json::Error> {
    serde_json::to_string_pretty(usage)
}

/// Formats a slice of [`ProviderUsage`] snapshots as a CSV string.
