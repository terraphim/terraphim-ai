...
impl FcctlVmManagerAdapter {
    ...
    pub fn new(
        ...
        let inner =
            FcctlVmManager::new(&firecracker_bin, &socket_base_path, None).map_err(|e| {
                FcctlAdapterError::InitializationFailed {
    ...
    fn convert_vm(&self, fcctl_vm: &fcctl_core::firecracker::VmState) -> Vm {
        ...
        let created_at: DateTime<Utc> = fcctl_vm.created_at.parse().unwrap_or_else(|_| Utc::now());
    ...
    fn translate_error(e: fcctl_core::Error, context: impl Into<String>) -> FcctlAdapterError {
    ...
    pub async fn get_vm_client(
        &self,
        vm_id: &str,
    ) -> anyhow::Result<fcctl_core::firecracker::FirecrackerClient> {
        // Create a Firecracker client connected to the VM's socket
        let socket_path = self.socket_base_path.join(format!("{}.sock", vm_id));
        let client =
            fcctl_core::firecracker::FirecrackerClient::new(&socket_path, Some(vm_id.to_string()));
        Ok(client)
    }
}
    ...
    async fn create_vm(&self, _config: &VmConfig) -> anyhow::Result<Vm> {
        ...
        let fcctl_config = FcctlVmConfig {
            ...
            kernel_path: _config
                .kernel_path
                .clone()
                .unwrap_or_else(|| self.kernel_path.to_string_lossy().to_string()),
            rootfs_path: _config
                .rootfs_path
                .clone()
                .unwrap_or_else(|| self.rootfs_path.to_string_lossy().to_string()),
            initrd_path: None,
        ...
        let created_vm_id = inner
            .create_vm(&fcctl_config, None)
            .await
            .map_err(|e| Self::translate_error(e, "Failed to create VM"))?;
