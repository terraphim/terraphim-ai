...
impl BuildConfig {
    ...
    pub fn from_file(path: &str) -> Result<Self> {
        ...
        if path.ends_with(".toml") {
            toml::from_str(&content).map_err(|e| Error::ParseError(format!("TOML parse error: {}", e)))
        } else if path.ends_with(".yaml") || path.ends_with(".yml") {
            serde_yaml::from_str(&content).map_err(|e| Error::ParseError(format!("YAML parse error: {}", e)))
        } else if path.ends_with(".json") {
            serde_json::from_str(&content).map_err(|e| Error::ParseError(format!("JSON parse error: {}", e)))
        } else {
            Err(Error::UnsupportedFormat(format!("Unsupported config format: {}", path)))
        }
    }
    ...
    pub fn to_file(&self, path: &str) -> Result<()> {
        let content = if path.ends_with(".toml") {
            toml::to_string_pretty(self).map_err(|e| Error::SerializationError(format!("TOML serialization error: {}", e)))?
        } else if path.ends_with(".yaml") || path.ends_with(".yml") {
            serde_yaml::to_string(self).map_err(|e| Error::SerializationError(format!("YAML serialization error: {}", e)))?
        } else if path.ends_with(".json") {
            serde_json::to_string_pretty(self).map_err(|e| Error::SerializationError(format!("JSON serialization error: {}", e)))?
        } else {
            return Err(Error::UnsupportedFormat(format!("Unsupported config format: {}", path)));
        };
