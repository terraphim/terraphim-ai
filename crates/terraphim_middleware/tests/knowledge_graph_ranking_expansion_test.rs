...
async fn test_knowledge_graph_ranking_expansion() {
    ...
    assert!(original_kg_path.exists(), "Knowledge graph directory should exist: {:?}", original_kg_path);
    assert!(original_kg_path.join("terraphim-graph.md").exists(), "terraphim-graph.md should exist");
    
    // 2. Create temporary directory and copy existing KG files
    let temp_dir = TempDir::new().expect("Failed to create temp directory");
    let temp_kg_path = temp_dir.path().join("kg");
    fs::create_dir_all(&temp_kg_path).await.expect("Failed to create temp kg dir");
    
    // Copy existing KG files to temp directory
    let mut original_files = Vec::new();
    let mut entries = fs::read_dir(&original_kg_path).await.expect("Failed to read kg directory");
    while let Some(entry) = entries.next_entry().await.expect("Failed to read entry") {
        if entry.path().extension().map_or(false, |ext| ext == "md") {
            ...
            fs::copy(&source_path, &dest_path).await.expect("Failed to copy file");
            original_files.push(file_name.to_string_lossy().to_string());
        }
    }
    
    println!("📁 Copied {} original KG files to temp directory", original_files.len());
    ...
    println!("📊 Initial thesaurus contains {} terms", initial_thesaurus_size);
    ...
    let mut entries = fs::read_dir(&temp_kg_path).await.expect("Failed to read temp kg directory");
    while let Some(entry) = entries.next_entry().await.expect("Failed to read entry") {
        if entry.path().extension().map_or(false, |ext| ext == "md") {
            let content = fs::read_to_string(&entry.path()).await.expect("Failed to read file");
...
"#;
    ...
    fs::write(&new_kg_file_path, new_kg_content).await
    ...
    println!("📊 Expanded thesaurus contains {} terms", expanded_thesaurus_size);
    ...
    assert!(expanded_thesaurus_size > initial_thesaurus_size, 
        ...
        initial_thesaurus_size, expanded_thesaurus_size);
    println!("✅ Thesaurus grew: {} -> {} terms (+{})", 
        initial_thesaurus_size, expanded_thesaurus_size, 
    ...
    assert!(expanded_nodes_count > initial_nodes_count, 
        ...
        initial_nodes_count, expanded_nodes_count);
    println!("✅ Nodes increased: {} -> {} (+{})", 
        initial_nodes_count, expanded_nodes_count, 
    ...
    assert!(expanded_edges_count > initial_edges_count, 
        ...
        initial_edges_count, expanded_edges_count);
    println!("✅ Edges increased: {} -> {} (+{})", 
        initial_edges_count, expanded_edges_count, 
        expanded_edges_count - initial_edges_count);
    
    // Verify documents increased
    assert!(expanded_documents.len() > initial_documents.len(), 
        "Documents should have increased from {} to {}", 
        initial_documents.len(), expanded_documents.len());
    println!("✅ Documents increased: {} -> {} (+{})", 
        initial_documents.len(), expanded_documents.len(), 
    ...
    println!("✅ Rank changed: {} -> {} ({}{})", 
        initial_rank, expanded_rank,
        if expanded_rank > initial_rank { "+" } else { "" },
    ...
    for synonym in &new_synonym_tests {
        ...
        assert!(!results.is_empty(), 
            "Should find results for new synonym: '{}'", synonym);
        println!("✅ Found {} results for synonym: '{}'", results.len(), synonym);
    }
    
    // 12. Verify Terraphim Engineer role configuration is used correctly
    assert_eq!(role_name.original.as_str(), "Terraphim Engineer", 
