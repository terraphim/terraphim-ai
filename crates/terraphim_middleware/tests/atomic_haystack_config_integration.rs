...
async fn test_atomic_haystack_with_terraphim_config() {
    ...
    let config = ConfigBuilder::new()
        ...
        .add_role("AtomicUser", {
            let mut role = Role::new("AtomicUser");
            role.shortname = Some("AtomicUser".to_string());
...
async fn test_atomic_haystack_public_vs_authenticated_access() {
    ...
    let config = ConfigBuilder::new()
        .global_shortcut("Ctrl+T")
        .add_role("MixedAccessUser", {
            let mut role = Role::new("MixedAccessUser");
            role.shortname = Some("MixedAccessUser".to_string());
            role.relevance_function = RelevanceFunction::TitleScorer;
            role.theme = "spacelab".to_string();
