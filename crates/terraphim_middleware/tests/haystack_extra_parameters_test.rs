...
async fn test_haystack_serialization_security() {
    ...
    let mut ripgrep_haystack = Haystack::new(
    ...
    assert!(!ripgrep_json.contains("secret_should_not_appear"), 
            "Ripgrep haystack should NOT serialize atomic_server_secret");
    assert!(!ripgrep_json.contains("atomic_server_secret"), 
    ...
    assert!(atomic_json.contains("valid_atomic_secret"), 
    ...
    println!("Atomic haystack (no secret) JSON: {}", atomic_no_secret_json);
    
    assert!(!atomic_no_secret_json.contains("atomic_server_secret"), 
...
async fn test_ripgrep_extra_parameters() {
    ...
    assert!(unknown_args.is_empty(), "Unknown parameters should not generate arguments");
...
async fn test_haystack_builder_and_extra_parameters() {
    ...
    println!("Builder created haystack with {} extra parameters", params.len());
    ...
    assert_eq!(atomic_haystack.atomic_server_secret, Some("test_secret".to_string()));
    assert_eq!(atomic_haystack.get_extra_parameters().get("timeout"), Some(&"30".to_string()));
...
async fn test_ripgrep_indexer_with_extra_parameters() {
    ...
    match result {
        Ok(index) => {
            println!("Indexer with extra parameters returned {} documents", index.len());
    ...
    match simple_result {
        Ok(index) => {
            println!("Indexer without extra parameters returned {} documents", index.len());
        }
        Err(e) => {
...
async fn test_haystack_serialization_completeness() {
    ...
    let ripgrep_with_params = Haystack::new(
        "docs/".to_string(),
        ServiceType::Ripgrep,
        false,
    )
    ...
    let minimal_haystack = Haystack::new(
        "minimal/".to_string(),
        ServiceType::Ripgrep,
        true,
    );
...
async fn test_tag_filtering_use_cases() {
    ...
    let rust_dev_haystack = Haystack::new(
        "src/".to_string(),
        ServiceType::Ripgrep,
        false,
    )
    ...
    println!("  Tag filter: {:?}", rust_dev_haystack.get_extra_parameters().get("tag"));
    println!("  Type filter: {:?}", rust_dev_haystack.get_extra_parameters().get("type"));
    ...
    println!("  Tag filter: {:?}", docs_haystack.get_extra_parameters().get("tag"));
    println!("  Context lines: {:?}", docs_haystack.get_extra_parameters().get("context"));
    ...
    println!("  Tag filter: {:?}", test_haystack.get_extra_parameters().get("tag"));
    println!("  Case sensitive: {:?}", test_haystack.get_extra_parameters().get("case_sensitive"));
    println!("  Max results: {:?}", test_haystack.get_extra_parameters().get("max_count"));
    ...
    for (name, haystack) in [
        ...
        assert!(!json.contains("atomic_server_secret"), 
                "{} haystack should not serialize atomic_server_secret", name);
        assert!(json.contains("extra_parameters"), 
                "{} haystack should serialize extra_parameters", name);
    }
