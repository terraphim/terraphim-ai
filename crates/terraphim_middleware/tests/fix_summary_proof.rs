...
fn test_fix_summary_proof() {
    ...
    for (case_name, example_input, old_problematic_pattern) in problematic_cases {
        ...
        assert!(
            !new_clean_id.contains("http"),
            "New ID should not contain URLs"
        );
        assert!(
            new_clean_id
                .chars()
        ...
        let improvement_ratio = (old_problematic_pattern.len() - new_clean_id.len()) * 100
            / old_problematic_pattern.len();
        println!(
            "  ✅ IMPROVEMENT: {}% shorter and filesystem-safe",
            improvement_ratio
        );
    }
