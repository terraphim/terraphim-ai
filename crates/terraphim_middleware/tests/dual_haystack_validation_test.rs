...
async fn test_dual_haystack_comprehensive_validation() {
        ...
        match *role_name {
            "DualTitle" | "DualGraph" => {
                assert_eq!(
                    role.haystacks.len(),
                    2,
                    "Dual roles should have 2 haystacks"
                );
                assert!(role
                    .haystacks
                    .iter()
            ...
            "SingleAtomic" => {
                assert_eq!(
                    role.haystacks.len(),
                    1,
                    "Single atomic role should have 1 haystack"
                );
            ...
            "SingleRipgrep" => {
                assert_eq!(
                    role.haystacks.len(),
                    1,
                    "Single ripgrep role should have 1 haystack"
                );
                    ...
                    ServiceType::Atomic => {
                        match atomic_indexer.index(search_term, haystack).await {
                            Ok(results) => {
                                ...
                                log::debug!(
                                    "Role {}, Atomic haystack, term '{}': {} results",
                                    role_name,
                                    search_term,
                                    atomic_results
                                );
                            ...
                            Err(e) => {
                                log::warn!(
                                    "Atomic search failed for role {}, term '{}': {}",
                                    role_name,
                                    search_term,
                                    e
                                );
                    ...
                    ServiceType::Ripgrep => {
                        match ripgrep_indexer.index(search_term, haystack).await {
                            Ok(results) => {
                                ...
                                log::debug!(
                                    "Role {}, Ripgrep haystack, term '{}': {} results",
                                    role_name,
                                    search_term,
                                    ripgrep_results
                                );
                            ...
                            Err(e) => {
                                log::warn!(
                                    "Ripgrep search failed for role {}, term '{}': {}",
                                    role_name,
                                    search_term,
                                    e
                                );
    ...
    for search_term in &search_terms {
        let dual_title_results = all_results
            .get("DualTitle")
            .and_then(|r| r.get(*search_term))
            .map(|(total, atomic, ripgrep, duration)| (*total, *atomic, *ripgrep, *duration));

        let single_atomic_results = all_results
            .get("SingleAtomic")
            .and_then(|r| r.get(*search_term))
            .map(|(total, atomic, ripgrep, duration)| (*total, *atomic, *ripgrep, *duration));

        let single_ripgrep_results = all_results
            .get("SingleRipgrep")
            .and_then(|r| r.get(*search_term))
            .map(|(total, atomic, ripgrep, duration)| (*total, *atomic, *ripgrep, *duration));

        if let (
            Some((dual_total, dual_atomic, dual_ripgrep, dual_duration)),
            Some((single_atomic_total, _, _, single_atomic_duration)),
            Some((single_ripgrep_total, _, _, single_ripgrep_duration)),
        ) = (
            dual_title_results,
            single_atomic_results,
            single_ripgrep_results,
        ) {
            ...
            log::info!(
                "  Single Atomic: {} in {:?}",
                single_atomic_total,
                single_atomic_duration
            );
            log::info!(
                "  Single Ripgrep: {} in {:?}",
                single_ripgrep_total,
                single_ripgrep_duration
            );
            log::info!(
                "  Combined Single: {} total",
                single_atomic_total + single_ripgrep_total
            );
    ...
    for (role_name, config) in &configs {
        if role_name.starts_with("Dual") {
            log::info!(
                "Testing search pipeline integration for role: {}",
                role_name
            );
            ...
            for search_term in &search_terms[..3] {
                ...
                match search_results {
                    Ok(results) => {
                        log::info!(
                            "  Pipeline search for '{}' found {} results in {:?}",
                            search_term,
                            results.len(),
                            pipeline_duration
                        );
...
async fn test_dual_haystack_config_validation() {
    ...
    let config: terraphim_config::Config =
        serde_json::from_str(&config_content).expect("Failed to parse dual haystack config");

    // Validate configuration structure
    assert_eq!(
        config.roles.len(),
        5,
        "Should have 5 roles in dual haystack config"
    );

    // Validate dual haystack roles
    let dual_title_role = config
        .roles
        .get(&"Dual Haystack Title Scorer".into())
        .expect("Dual Haystack Title Scorer role should exist");
    assert_eq!(
        dual_title_role.relevance_function,
        RelevanceFunction::TitleScorer
    );
    assert!(
        dual_title_role.kg.is_none(),
        "Title scorer should not have knowledge graph"
    );
    assert_eq!(
        dual_title_role.haystacks.len(),
        2,
        "Should have 2 haystacks"
    );
    assert!(dual_title_role
        .haystacks
        .iter()
        .any(|h| h.service == ServiceType::Atomic));
    assert!(dual_title_role
    ...
    let dual_graph_role = config
        .roles
        .get(&"Dual Haystack Graph Embeddings".into())
        .expect("Dual Haystack Graph Embeddings role should exist");
    assert_eq!(
        dual_graph_role.relevance_function,
        RelevanceFunction::TerraphimGraph
    );
    assert!(
        dual_graph_role.kg.is_some(),
        "Graph embeddings should have knowledge graph"
    );
    assert_eq!(
        dual_graph_role.haystacks.len(),
        2,
        "Should have 2 haystacks"
    );
    assert!(dual_graph_role
        .haystacks
        .iter()
        .any(|h| h.service == ServiceType::Atomic));
    assert!(dual_graph_role
    ...
    let hybrid_role = config
        .roles
        .get(&"Dual Haystack Hybrid Researcher".into())
        .expect("Dual Haystack Hybrid Researcher role should exist");
    assert_eq!(
        hybrid_role.relevance_function,
        RelevanceFunction::TerraphimGraph
    );
    assert!(
        hybrid_role.kg.is_some(),
        "Hybrid researcher should have knowledge graph"
    );
    ...
    assert!(
        hybrid_role
            .haystacks
            .iter()
            .filter(|h| h.service == ServiceType::Ripgrep)
            .count()
            == 2
    );

    // Validate single haystack reference roles
    let single_atomic_role = config
        .roles
        .get(&"Single Atomic Reference".into())
        .expect("Single Atomic Reference role should exist");
    assert_eq!(
        single_atomic_role.haystacks.len(),
        1,
        "Should have 1 haystack"
    );
    assert_eq!(single_atomic_role.haystacks[0].service, ServiceType::Atomic);

    let single_ripgrep_role = config
        .roles
        .get(&"Single Ripgrep Reference".into())
        .expect("Single Ripgrep Reference role should exist");
    assert_eq!(
        single_ripgrep_role.haystacks.len(),
        1,
        "Should have 1 haystack"
    );
    assert_eq!(
        single_ripgrep_role.haystacks[0].service,
        ServiceType::Ripgrep
    );
...
async fn test_source_differentiation_validation() {
    ...
    if !atomic_docs.is_empty() {
        let has_atomic_prefix = atomic_docs
            .iter()
            .any(|doc| doc.title.starts_with("ATOMIC:"));
    ...
    if !ripgrep_docs.is_empty() {
        let has_markdown_titles = ripgrep_docs
            .iter()
            .any(|doc| !doc.title.starts_with("ATOMIC:"));
        if has_markdown_titles {
            log::info!(
                "✅ Found ripgrep documents without ATOMIC: prefix for source differentiation"
            );
        }
    }
