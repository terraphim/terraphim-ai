...
mod tests {
    ...
    async fn test_memory_only_device_settings() -> Result<()> {
        ...
        assert_eq!(settings.profiles.get("memory").unwrap().get("type").unwrap(), "memory");
    ...
    async fn test_memory_persistable() -> Result<()> {
        ...
        impl TestData {
            ...
            async fn load_from_memory_operator(name: &str, operator: &opendal::Operator) -> Result<Self> {
    ...
    async fn test_thesaurus_memory_persistence() -> Result<()> {
        ...
        thesaurus.insert(NormalizedTermValue::from("artificial intelligence"), term2.clone());
        ...
        let ml_term = loaded_thesaurus.get(&NormalizedTermValue::from("ml")).unwrap();
        assert_eq!(ml_term.id, 1);
        assert_eq!(ml_term.value, NormalizedTermValue::from("machine learning"));
        
        let ai_term = loaded_thesaurus.get(&NormalizedTermValue::from("ai")).unwrap();
        assert_eq!(ai_term.id, 2);
        assert_eq!(ai_term.value, NormalizedTermValue::from("artificial intelligence"));
    ...
    async fn test_config_memory_persistence() -> Result<()> {
        ...
        let key = format!("{}_config.json", match config.id {
