...
async fn test_quick_validation_all_features() -> Result<()> {
    ...
    assert_eq!(key, "thesaurus_testengineer.json", "Thesaurus key should be normalized correctly");
    ...
    assert_eq!(loaded_thesaurus.len(), 1, "Loaded thesaurus should have 1 entry");
    assert_eq!(loaded_thesaurus.name(), "Test Engineer", "Loaded thesaurus name should match");
    ...
    document.id = "Test Document ID".to_string();
    document.title = "Test Document".to_string();
    document.body = "Test content".to_string();
    
    // Validate key generation
    let doc_key = document.get_key();
    assert_eq!(doc_key, "document_testdocumentid.json", "Document key should be normalized correctly");
    println!("  ✅ Key generation: 'Test Document ID' → '{}'", doc_key);
    
    // Test save/load
    document.save_to_one("memory").await?;
    let mut loaded_document = Document::default();
    loaded_document.id = "Test Document ID".to_string();
    loaded_document = loaded_document.load().await?;
    
    assert_eq!(loaded_document.title, "Test Document", "Loaded document title should match");
    assert_eq!(loaded_document.body, "Test content", "Loaded document body should match");
    println!("  ✅ Save/load cycle successful");
    ...
    for (input, expected) in challenging_names {
        ...
        assert_eq!(key, expected_key, "Key normalization failed for '{}'", input);
        
        let mut doc = Document::default();
        doc.id = input.to_string();
        let doc_key = doc.get_key();
        let expected_doc_key = format!("document_{}.json", expected);
        
        assert_eq!(doc_key, expected_doc_key, "Document key normalization failed for '{}'", input);
        
        println!("  ✅ Normalization consistent: '{}' → '{}'", input, expected);
    }
