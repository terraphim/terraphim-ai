    ...
    for size in &[100, 500, 1000, 2500, 5000, 10000] {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                let thesaurus = create_large_thesaurus(size);
                b.iter(|| {
                    build_autocomplete_index(thesaurus.clone(), None).unwrap()
                })
            },
        );
    }
...
fn bench_search_throughput(c: &mut Criterion) {
    ...
    for (query_name, query) in test_queries {
        ...
        group.bench_with_input(
            ...
            &query,
            |b, &query| {
                b.iter(|| autocomplete_search(&index, query, Some(10)).unwrap())
            },
        );
    }
    group.finish();
...
fn bench_search_result_limits(c: &mut Criterion) {
    ...
    for limit in &[1, 5, 10, 25, 50, 100, 250] {
        group.bench_with_input(
            ...
            limit,
            |b, &limit| {
                b.iter(|| autocomplete_search(&index, query, Some(limit)).unwrap())
            },
        );
    }
    group.finish();
...
fn bench_serialization_throughput(c: &mut Criterion) {
    ...
    for size in &[100, 500, 1000, 2500, 5000] {
        ...
        group.bench_with_input(
            ...
            &index,
            |b, index| {
                b.iter(|| serialize_autocomplete_index(index).unwrap())
            },
        );
        ...
        group.bench_with_input(
            ...
            &serialized,
            |b, data| {
                b.iter(|| deserialize_autocomplete_index(data).unwrap())
            },
        );
    }
    group.finish();
...
fn bench_realistic_usage(c: &mut Criterion) {
    ...
    let typing_patterns = vec![
        ...
        vec!["a", "ar", "art", "arti", "artif", "artifi", "artifici", "artificial"],
        vec!["d", "da", "dat", "data"],
