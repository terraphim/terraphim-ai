# Compliance Audit Report: 2026-04-29 15:58 CEST

**Status**: PASS

**Agent**: Vigil (compliance-watchdog)
**Date**: 2026-04-29
**Audit Type**: Manual review triggered for issue #815

---

## Executive Summary

Compliance audit of terraphim-ai project completed. No critical or high severity findings detected. All dependency security advisories previously flagged remain resolved. License compliance verified across workspace crates. Data protection patterns unchanged -- no PII collection, secrets appropriately handled.

## 1. Licence Compliance: PASS

**Command**: `cargo deny check licences`
**Result**: `licences ok`
...
|-------|--------|----------|
| Workspace crate licences | PASS | All crates declare permitted licences |
| Dependency tree licences | PASS | All transitive dependencies use allowed licences per deny.toml |
| html2md SPDX identifier | WARN | Uses deprecated "GPL-3.0+" identifier (external dep, not blocking) |
| Unused licence allowances | INFO | OpenSSL, Unicode-DFS-2016 in deny.toml not currently matched |

**Assessment**: No licence violations detected. The deprecated SPDX identifier in html2md is a warning-only issue in cargo-deny and does not block compliance.
...
**Assessment**: All previously flagged critical and high severity advisories remain resolved through dependency updates. Five unmaintained crate warnings persist; all are documented in deny.toml with accepted risk rationale.
...
| Secret serialisation | Conditional for Atomic service only | PASS | `atomic_server_secret` only serialised when `service == Atomic` |
...
|-----------|--------|-------|
| Licence Compliance | PASS | All crates and dependencies permitted |
...
---
*Report generated by Vigil Security Audit - 2026-04-29 15:58 CEST*
