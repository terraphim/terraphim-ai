...
wget -q "https://github.com/terraphim/terraphim-ai/releases/download/$RELEASE_VERSION/terraphim-agent_${RELEASE_VERSION#v}-1_amd64.deb"
...
wget -q "https://github.com/terraphim/terraphim-ai/releases/download/$RELEASE_VERSION/terraphim-agent-${RELEASE_VERSION#v}-1-x86_64.pkg.tar.zst"
...
dpkg-deb -I terraphim-agent_${RELEASE_VERSION#v}-1_amd64.deb | grep -E "(Package|Version|Architecture)"
...
tar -I 'zstd -d' -tf terraphim-agent-${RELEASE_VERSION#v}-1-x86_64.pkg.tar.zst | grep -E "(terraphim-agent|.PKGINFO)" | head -5
...
if command -v dpkg >/dev/null 2>&1; then
    ...
    dpkg-deb --contents terraphim-agent_${RELEASE_VERSION#v}-1_amd64.deb | grep -E "(usr/bin)"
fi
