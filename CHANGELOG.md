...
- **Apple Container RLM backend** `terraphim_rlm` gains `BackendType::AppleContainer` and an argv-safe, session-affine `AppleContainerExecutor` driving Apple's `container` CLI (one lightweight Linux VM per container on Apple silicon / macOS 26). Feature-gated by `apple-container-backend` (included in `full`, no new dependency); availability requires macOS/aarch64 plus a healthy `container system version`/`status` and never auto-starts the host service; timeouts kill the CLI child and destroy the session container, as does a `container exec` that fails with a host I/O error (which can be raised after the child was spawned, so it is no proof that no guest process ran). Exactly one `container exec` is issued per call — a session container reported missing is discarded and the failure returned, never re-run — and session teardown carries an explicit lifecycle state plus an executor-wide gate so no interleaving can leave an untracked container behind: `end_session` is terminal for a session id (a retained tombstone stops any concurrent or later resurrection), deletion failures propagate to the caller — including through the public `TerraphimRlm::destroy_session` — and stay tracked for `cleanup` to retry and aggregate, a name is tracked before `container run` can create anything and blocks a replacement until its deletion is confirmed, and execution ownership is runtime-independent from the start: once the lifecycle permit is held and the container is resolved — and **before the guest command is launched** — the permit and the whole `container exec` operation are moved into a dedicated owner thread driving its own current-thread runtime, which owns the CLI child, its stdout/stderr drains, the outcome and every recovery, while the caller only awaits a result channel. If that runtime cannot be built or the thread cannot be spawned, the call returns `BackendInitFailed` before any `container exec`, so no guest command was launched and there is nothing to recover (any session container created by the preceding resolution stays tracked and reusable). Cancelling an execution future therefore fails closed by construction: the drop claims the outcome, quarantines the session slot and signals the CLI call — it spawns nothing and hands nothing over — and the pre-established owner awaits the process runner's return unconditionally (it never aborts or drops the future holding the child and drain handles, since only that return proves the child was killed **and reaped** and the readers stopped; a runner that violates this contract holds the permit indefinitely and blocks `cleanup` fail-closed rather than reporting an unobserved terminal state), then force-deletes the container and only then releases the permit, even when the future is dropped on a non-runtime thread and even when the runtime that started the execution has already been shut down (that runtime never owned the child, the drains or the permit). Lifecycle coordination is a single invariant rather than a protocol: an owned lifecycle read permit is acquired **before** an operation can create, use or reclaim a container — before `container run`, before `container exec` — and is held, by the owner of the work, until that operation and every recovery it triggers has finished (the permit never leaves that owner, so there is no spawn/register gap on any path). `cleanup` sets the closing flag and takes the write side, and acquiring it is by itself the proof that every earlier execution, creation, teardown and recovery has completed — so terminal cleanup cannot overtake a recovery that has not started yet, and its drained map cannot be repopulated after it returns, by a recovery, a later caller or an `end_session` (which checks the closing flag after taking its permit, while still tearing down any container that is already tracked). There is no recovery registry and no bounded join rounds; the trade-off is that `cleanup` waits for in-flight executions. Per-container CPU and memory are configurable via `apple_container_cpus`/`apple_container_memory` (validated, defaulting to `1`/`512M`). See `crates/terraphim_rlm/docs/apple-container-backend.md` (Refs #3192, 2026-08-10)
