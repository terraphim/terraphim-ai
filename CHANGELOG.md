...
- **Workflow handler documentation** — `execute_prompt_chain`, `execute_routing`, `execute_parallel`, `execute_orchestration`, `execute_optimization`, `execute_vm_execution_demo`, and `MultiAgentWorkflowExecutor::execute_vm_execution_demo`; `WebSocketSession` struct and all WebSocket broadcast/notify utilities in `websocket.rs`
