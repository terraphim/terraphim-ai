#!/usr/bin/env bash
# Post-Change Evaluation Script
# Usage: ./post-evaluation.sh <codebase_path> [role_name]
...
echo "=== Post-Change Evaluation ==="
echo "Codebase: $CODEBASE_PATH"
echo "Role: $ROLE"
echo "Output: $OUTPUT_DIR"
echo ""

# Run evaluation queries (same as baseline)
...
if [ -f "$CODEBASE_PATH/Cargo.toml" ]; then
    ...
    if command -v cargo &> /dev/null; then
        cargo clippy --all-targets -- -D warnings 2>&1 | tee "$OUTPUT_DIR/../../clippy-after.log" || true

        grep -c "warning:" "$OUTPUT_DIR/../../clippy-after.log" > "$OUTPUT_DIR/clippy-warnings.txt" 2>/dev/null || echo "0" > "$OUTPUT_DIR/clippy-warnings.txt"
    fi

    cargo test --no-fail-fast 2>&1 | tee "$OUTPUT_DIR/../../test-after.log" || true
...
echo ""
echo "Post-change evaluation complete!"
