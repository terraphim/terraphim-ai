...
async fn test_terraphim_engineer_local_kg_integration() {
    ...
    if !config_path.exists() {
        log::warn!("Terraphim Engineer config not found at {:?}. Skipping test.", config_path);
        return;
    }
    ...
    log::info!("Config file first 100 chars: {}", &config_content[..config_content.len().min(100)]);
    ...
    assert_eq!(terraphim_engineer_role.relevance_function, 
               terraphim_types::RelevanceFunction::TerraphimGraph,
               "Terraphim Engineer should use TerraphimGraph");
    
    assert!(terraphim_engineer_role.kg.is_some(), 
    ...
    assert!(config_response.status().is_success(), "Config request should succeed");
    
    let config_json: ConfigResponse = config_response.json().await
        .expect("Failed to parse config response");
    
    assert_eq!(config_json.config.default_role, "Terraphim Engineer".into());
    assert!(config_json.config.roles.contains_key(&"Terraphim Engineer".into()));
    ...
    let search_json: SearchResponse = search_response.json().await
        .expect("Failed to parse search response");
    
    assert!(!search_json.results.is_empty(), 
            "Search should return results for 'terraphim' in Terraphim docs");
    
    log::info!("✅ Found {} search results for 'terraphim'", search_json.results.len());
    ...
    for term in &engineering_terms {
        ...
        if search_response.status().is_success() {
            ...
            log::info!("✅ Found {} results for '{}'", search_json.results.len(), term);
            ...
            log::warn!("⚠️  Search for '{}' returned status: {}", term, search_response.status());
        }
    }
        ...
        if let Ok(response) = search_response {
            if response.status().is_success() {
                if let Ok(search_json) = response.json::<SearchResponse>().await {
                    log::info!("✅ KG search for '{}': {} results", term, search_json.results.len());
                } else {
                    log::warn!("⚠️  Failed to parse KG search results for '{}'", term);
                }
            } else {
                log::warn!("⚠️  KG search for '{}' returned status: {}", term, response.status());
            }
        } else {
