...
async fn test_kg_term_to_document_lookup() {
    ...
    if !docs_src_path.exists() {
        log::warn!("Terraphim docs not found at {:?}. Skipping test.", docs_src_path);
        return;
    }
    ...
    if !kg_path.exists() {
        log::warn!("Terraphim KG directory not found at {:?}. Skipping test.", kg_path);
        return;
    }
    ...
    let config_data: ConfigResponse = config_response.json().await.expect("Failed to parse config response");
    log::info!("✅ Config retrieved successfully");
    log::info!("Available roles: {:?}", config_data.config.roles.keys().collect::<Vec<_>>());
    ...
    let kg_search_response = client
        .get(&format!("{}/roles/Terraphim Engineer/kg_search?term=haystack", base_url))
        .timeout(Duration::from_secs(15))
        .send()
        .await;
    
    assert!(kg_search_response.is_ok(), "KG search request failed: {:?}", kg_search_response);
    let kg_search_response = kg_search_response.unwrap();
    assert_eq!(kg_search_response.status(), 200, "KG search returned non-200 status");
    ...
    // Validate results
    assert!(search_data.total > 0, "Expected to find at least one document for 'haystack' term");
    
    // Check if we found the haystack.md document specifically
    let haystack_doc = search_data.results.iter().find(|doc| {
        doc.id.contains("haystack") || doc.title.to_lowercase().contains("haystack")
    });
    
    if let Some(doc) = haystack_doc {
        log::info!("✅ Found haystack document: id='{}', title='{}'", doc.id, doc.title);
        ...
        if has_synonyms {
            ...
            log::warn!("⚠️ Document doesn't contain expected synonyms, but that's okay for this test");
        }
    } else {
        log::warn!("⚠️ Didn't find specific haystack.md document, but found {} related documents", search_data.total);
    }
    ...
    let synonym_search_response = client
        .get(&format!("{}/roles/Terraphim Engineer/kg_search?term=service", base_url))
        .timeout(Duration::from_secs(15))
        .send()
        .await;
    ...
    if synonym_search_response.is_ok() {
        ...
        if synonym_response.status() == 200 {
            ...
            log::info!("Found {} documents for 'service' synonym", synonym_data.total);
    ...
    let empty_search_response = client
        .get(&format!("{}/roles/Terraphim Engineer/kg_search?term=nonexistentterm", base_url))
        .timeout(Duration::from_secs(10))
        .send()
        .await;
    ...
    if empty_search_response.is_ok() {
        ...
        if empty_response.status() == 200 {
            ...
            log::info!("Found {} documents for non-existent term (expected: 0)", empty_data.total);
            
            // This should return 0 results
            assert_eq!(empty_data.total, 0, "Expected 0 results for non-existent term");
...
async fn test_kg_term_lookup_invalid_role() {
    ...
    let invalid_role_response = client
        .get(&format!("{}/roles/NonExistentRole/kg_search?term=test", base_url))
        .timeout(Duration::from_secs(10))
        .send()
        .await;
    
    assert!(invalid_role_response.is_ok(), "Request should complete even with invalid role");
    let response = invalid_role_response.unwrap();
    
    // Should return an error status (400 or 404 or 500)
    assert!(response.status().is_client_error() || response.status().is_server_error(), 
            "Expected error status for invalid role, got: {}", response.status());
    
    log::info!("✅ Invalid role test completed - correctly returned error status: {}", response.status());
