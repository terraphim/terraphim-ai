...
async fn test_system_operator_remote_kg_integration() {
    ...
    if !system_operator_path.exists() {
        log::warn!("System operator data not found at {:?}. Run setup_system_operator.sh first.", system_operator_path);
        return;
    }
    ...
    if !config_path.exists() {
        log::warn!("System operator config not found at {:?}. Skipping test.", config_path);
        return;
    }
    ...
    assert!(config_json.config.roles.contains_key(&"System Operator".into()));
    ...
    log::info!("✅ Found {} search results for 'system'", search_json.results.len());
