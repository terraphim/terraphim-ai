...
async fn test_e2e_kg_haystack_lookup_comprehensive() {
    ...
    let rolegraph_sync = config_state
        .roles
        .get(&engineer_role)
    ...
    assert!(
        document_count > 0,
        "Role graph should contain documents from local KG"
    );
    ...
    for term in &test_terms {
        ...
        if term == &"haystack" {
            assert!(
                !results.is_empty(),
                "Should find documents for 'haystack' term"
            );
            ...
            Ok(response) => {
                ...
                if response.status().as_u16() == 200 {
                    match response.json::<SearchResponse>().await {
                        Ok(search_response) => {
                            println!(
                                "    📊 API returned {} documents for term '{}'",
    ...
    assert!(
        !unique_docs.is_empty(),
        "Should find at least one unique document"
    );
...
async fn test_kg_haystack_specific_document_validation() {
    ...
    println!(
        "✅ Synonym validation completed - found {} total document matches",
        total_found
    );
}
