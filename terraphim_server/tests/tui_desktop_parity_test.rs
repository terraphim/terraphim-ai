...
async fn test_tui_vs_direct_api_search_parity() {
    ...
    assert!(http_response.status().is_success(), "HTTP response should be successful");
    ...
    assert_eq!(tui_response.total, http_body["total"].as_u64().unwrap() as usize);
    assert_eq!(tui_response.results.len(), http_body["results"].as_array().unwrap().len());
...
async fn test_tui_vs_direct_api_config_parity() {
    ...
    let tui_role_names: HashSet<String> = tui_response.config.roles.keys()
        .map(|k| k.to_string())
        .collect();
...
async fn test_tui_vs_direct_api_role_switching_parity() {
    ...
    let roles: Vec<String> = config.config.roles.keys()
        .map(|k| k.to_string())
        .collect();
...
async fn test_tui_vs_direct_api_rolegraph_parity() {
    ...
    // Validate basic response structure
    assert_eq!(tui_response.status, http_body["status"].as_str().unwrap());
    assert_eq!(tui_response.nodes.len(), http_body["nodes"].as_array().unwrap().len());
    assert_eq!(tui_response.edges.len(), http_body["edges"].as_array().unwrap().len());
...
async fn test_pagination_parity() {
    ...
    assert_eq!(tui_p1.results.len(), http_page1["results"].as_array().unwrap().len());
    assert_eq!(tui_p2.results.len(), http_page2["results"].as_array().unwrap().len());

    // Verify pages contain different results (if enough data exists)
    if tui_p1.results.len() == limit && tui_p2.results.len() > 0 {
        ...
        assert!(p1_ids.is_disjoint(&p2_ids), "Pages should contain different documents");
    }
