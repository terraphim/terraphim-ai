...
async fn test_kg_lookup_functionality_basic() {
    ...
    let content = std::fs::read_to_string(&haystack_path)
        .expect("Failed to read haystack.md");
    ...
    assert!(content.contains("synonyms::"), "Document should contain synonyms");
    ...
    let test_doc = terraphim_types::Document {
        ...
        stub: None,
        tags: Some(vec!["datasource".to_string(), "service".to_string(), "agent".to_string()]),
        rank: None,
    };
...
async fn test_rolegraph_find_documents_for_term_direct() {
    ...
    let datasource_term = NormalizedTerm::new(2, NormalizedTermValue::new("datasource".to_string()));
    ...
    thesaurus.insert(NormalizedTermValue::new("haystack".to_string()), haystack_term);
    thesaurus.insert(NormalizedTermValue::new("datasource".to_string()), datasource_term);
    thesaurus.insert(NormalizedTermValue::new("service".to_string()), service_term);
    ...
    match RoleGraph::new(role_name, thesaurus).await {
        Ok(rolegraph) => {
            ...
            for term in test_terms {
                ...
                println!("🔎 Term '{}' found in {} documents: {:?}", term, document_ids.len(), document_ids);
            }
        }
        Err(e) => {
