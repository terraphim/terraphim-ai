...
async fn test_terraphim_graph_search_comprehensive() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for doc in &test_documents {
        ...
        assert_eq!(loaded_doc.title, doc.title, "Document title should be preserved");
        assert_eq!(loaded_doc.body, doc.body, "Document body should be preserved");
        assert!(!loaded_doc.body.is_empty(), "Document body should not be empty");
        log::info!("✅ Verified document '{}' can be loaded with content", doc.id);
    }
    ...
    let _thesaurus = terraphim_service.ensure_thesaurus_loaded(&role_name).await?;
    ...
    if let Some(rolegraph_sync) = config_state.roles.get(&role_name) {
        ...
        assert!(node_count > 0, "RoleGraph should have nodes after document indexing");
        assert!(edge_count > 0, "RoleGraph should have edges after document indexing");
        assert_eq!(document_count, test_documents.len(), "RoleGraph should contain all test documents");
    ...
    let test_queries = vec![
        ...
        ("knowledge graph", "Should find knowledge graph related documents"),
    ];
    ...
    for (query, description) in test_queries {
        ...
        for (i, result) in results.iter().enumerate() {
            log::info!("    {}. '{}' (ID: {}, Rank: {:?})", 
                      i + 1, result.title, result.id, result.rank);
        }
        ...
        if query == "haystack" || query == "service" || query == "terraphim-graph" {
            assert!(!results.is_empty(), 
                   "Query '{}' should return results - this indicates TerraphimGraph is working", query);
            log::info!("  ✅ Query '{}' successfully returned {} results", query, results.len());
        }
    }
    ...
    for doc in &test_documents {
        ...
        assert!(!loaded_doc.body.is_empty(), "Loaded document should have body content");
        
        log::info!("✅ Successfully loaded document '{}' with content via persistence", doc.id);
    }
...
async fn test_persistence_fallback_behavior() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(loaded_doc.title, test_doc.title, "Title should be preserved");
...
async fn test_empty_rolegraph_search() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(results.is_empty(), "Empty rolegraph should return empty results");
    
    log::info!("✅ Empty rolegraph search handled gracefully (returned {} results)", results.len());
    Ok(())
}